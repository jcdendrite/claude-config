...
`git pull` — no re-install needed. Exception: `settings.json` itself is
gitignored and generated by `render-settings.sh` from
`claude/.claude/settings.base.json`, so a `git pull` alone leaves the live
`~/.claude/settings.json` stale until `install.sh` re-runs.
