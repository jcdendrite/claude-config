...
# markers below and runs them under an isolated $HOME. Keep both markers on
# their own line, wrapping the whole block.
# INSTALL_TEST_FIXTURE: rc-block-helpers — start
# Defined here, ahead of the render call below, rather than down with
# ensure_local_bin_on_path: ensure_settings_render (next block) must call
# _ensure_rc_block before render-settings-invoke runs, and bash does not
# hoist function definitions -- a function must already be defined by the
# time execution reaches its call.
#
...
# INSTALL_TEST_FIXTURE: rc-block-helpers — end

# The hook test suite extracts the lines between the two INSTALL_TEST_FIXTURE
# markers below and runs them under an isolated $HOME. Keep both markers on
# their own line, wrapping the whole block.
# INSTALL_TEST_FIXTURE: settings-render-rc — start
...
ensure_settings_render() {
  ...
  # at install time.
  local render_invocation='[ -x "$HOME/.claude/scripts/ensure-settings-render.sh" ] && "$HOME/.claude/scripts/ensure-settings-render.sh"'
  local block_content shell_name

  printf -v block_content '\n# BEGIN claude-config: ensure settings.json render\n%s\n# END claude-config: ensure settings.json render\n' "$render_invocation"

  for shell_name in zsh bash; do
    _ensure_rc_block "$shell_name" 'ensure-settings-render.sh' 'the settings.json render check' "$render_invocation" "$block_content"
  done
}
# INSTALL_TEST_FIXTURE: settings-render-rc — end

# Placed immediately above render-settings-invoke, not down with the other
# top-level calls near the end of this script: the rc block must be
# installed before a render failure can abort the rest of install.sh under
# set -e, or a fresh install that fails its first render would never get the
# rc line that repairs it on the next new shell.
ensure_settings_render
...
# Calls _ensure_rc_block, defined in this file's earlier rc-block-helpers
# block -- extracting this block in isolation needs rc-block-helpers
# alongside it (see test_install_sh_local_bin_path.py's own extraction).
