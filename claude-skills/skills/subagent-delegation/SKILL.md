...
description: >
  ...
  on the same question; delegating implementation; delegating a multi-step
  gate/review loop. DO NOT TRIGGER when: single-artifact lookup (not a
  multi-site sweep); comprehension read feeding your own writing/review/design;
  Edit/Write while scope or content is forming; the failure output/diff you
