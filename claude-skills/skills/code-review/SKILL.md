...
Before anything else, track this invocation's **round number** — a 1-based count of `/code-review` invocations this session, including every prior Step 0.1 short-circuit — and pass the same value to every `review-ledger.sh append` call this round, so the ledger's round sequence never drifts from how many times this skill actually ran.
