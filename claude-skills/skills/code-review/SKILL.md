...
Evaluate the diff against CLAUDE.md §Engineering Judgment, §Working Style, and §Durable text — being loaded is not the same as being applied. The tripwires below fire on **diff surface**, not on internal reasoning, so they catch judgment-class defects even when the author's reasoning reads as coherent:
...
12a. **Comment/prose discipline on added or modified text** — Does a new or modified comment or durable-doc paragraph violate CLAUDE.md §Durable text or §Engineering Judgment's single-source-of-truth bullet?
   ...
   - Survives-the-PR self-test on durable-doc content — a durable-doc paragraph that fails it
   - Restated canonical rule — a record-style doc paragraph restating a rule stated canonically elsewhere instead of citing it

   Distinct from item 12, which covers comment *deletion*. Step 1.5's "Non-durable comment" tripwire runs only the non-durable-comment part of this check inline on every review, and `comment-discipline-reviewer` (Ripple effect triage) enumerates the rest when spawned.
...
On a re-review, prior decisions are this session's context plus every disposition record `ready-for-review/SKILL.md` § "3. Code review (halt on findings)" wrote for this branch; put those record paths and these three rules in each spawn prompt (a reviewer sees only its prompt). The spawn finishes its own review before opening the records — a record never narrows what it reviews or suppresses a finding the current text supports, and it names in its findings any record it cannot parse, or whose claim the current text contradicts. For each earlier ADDRESS row the same agent raised whose Outcome names a fix, it confirms the fix landed as the finding required; a missing or partial fix is a new finding for any reviewer that sees it, whichever agent raised the row. It re-flags a site an earlier fix or verdict rewrote only under a different rule than the one behind the rewrite, or for a fact the rewrite dropped, and when its fix would move a site back toward its earlier wording it names the finding it contradicts.
...
| Adds or modifies a comment or durable-doc prose beyond a hygiene tweak (code comments, docstrings, `REFERENCES.md`, doc files, README sections, skill/agent bodies) | `comment-discipline-reviewer` — enumerate every site violating CLAUDE.md §Durable text or §Engineering Judgment's single-source-of-truth bullet (comment verbosity, multi-fact comment structure, wrong-altitude prose, PR-defined terminology, "used to be X" framing, durable-doc self-test failures, restated canonical rule). Step 1.5's inline "Non-durable comment" tripwire still runs unconditionally regardless of this spawn. This reviewer carries no `Bash`; resolve its diff artifact per the note directly below this table before spawning it. Step 0.6 defers this row out of staged-diff commit-gate rounds. |
...
**Resolving `comment-discipline-reviewer`'s diff artifact.** Reuse your invoker's diff-file path if it handed you one. Otherwise, for a currently-staged diff, self-resolve by running `~/.claude/scripts/pr-diff-against-base.sh --staged --diff-file > /dev/null` as its own Bash call and take the path its `DIFF_FILE:` line names. The redirect keeps the diff's bytes out of this session's context. Two cases fall back the same way: (1) a `DIFF_EMPTY:` line, meaning nothing is staged; (2) a pasted-snippet or ad-hoc review with no git diff behind it at all. Either way, hand the spawn the diff text you do have, verbatim, and name the missing artifact on the `Spawn decisions:` line. Neither is a write failure. A genuine write failure means: quote the script's own stderr line, spawn nothing this round, and halt the review.
...
A finding DEFERred under the closed list, or settled *keep current text* by a contradiction consult, counts as resolved. If the review is **clean** (no blockers, no unresolved critical findings, and you reviewed the currently staged changes), record it by running this command exactly once:
