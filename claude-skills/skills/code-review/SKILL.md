...
| Adds or modifies a comment or durable-doc prose beyond a hygiene tweak (code comments, docstrings, `REFERENCES.md`, doc files, README sections, skill/agent bodies) | `comment-discipline-reviewer` — enumerate every site violating CLAUDE.md §Code Comments, Documentation, and Prose (comment verbosity, multi-fact comment structure, wrong-altitude prose, PR-defined terminology, "used to be X" framing, durable-doc self-test failures). Step 1.5's inline "Non-durable comment" tripwire still runs unconditionally regardless of this spawn. This reviewer carries no `Bash`; resolve a diff artifact before spawning it and pass the path in the prompt. Reuse your invoker's diff-file path if it handed you one. Otherwise, for a currently-staged diff, self-resolve by running `~/.claude/scripts/pr-diff-against-base.sh --staged --diff-file > /dev/null` as its own Bash call and take the path its `DIFF_FILE:` line names. The redirect keeps the diff's bytes out of this session's context. Two cases fall back the same way: (1) a `DIFF_EMPTY:` line, meaning nothing is staged; (2) a pasted-snippet or ad-hoc review with no git diff behind it at all. Either way, hand the spawn the diff text you do have, verbatim, and name the missing artifact on the `Spawn decisions:` line. Neither is a write failure. A genuine write failure means: quote the script's own stderr line, spawn nothing this round, and halt the review. |
