...
Before anything else, track this invocation's **round number**: the count of `/code-review` invocations you've made in this session, counting this one and every prior Step 0.1 short-circuit. It is 1-based, starting at 1 for the first invocation and incrementing by one on each subsequent one. Every `review-ledger.sh append` call this round, including this step's own below, uses that same number, so the ledger's own round sequence never drifts from how many times this skill actually ran.
