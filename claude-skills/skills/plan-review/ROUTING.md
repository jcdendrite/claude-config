...
When you spawn: pick the specialist that serves the question (table below is reference, not roster) and pass plan scope, section, specific question, **Item ownership** routing, and — when the plan carries an assumption ledger — the **Ledger cross-check** instruction's own wording copied into the spawn prompt as literal text. A pointer or section reference to `## Ledger cross-check` below is not sufficient: it either forces the reviewer to open this whole file to resolve it, or leaves the reviewer running the wrong check. For re-review rounds, pass the prior round's findings-file paths under `agent-reviews/`, falling back to those findings inline when the prior round wrote none — an older round, a denied write, or a parent worktree created after that round ran. What's been applied stays inline regardless: `marker.sh write plan-review` records only a content hash of the plan set, so there is no on-disk artifact for that half to point at. Reviewers without prior context re-discover; that's wasted spawn.
...
Before the first spawn this round, run `~/.claude/scripts/findings-path-suffix.sh` once and reuse its printed `<suffix>` (its last line of output; discard any earlier line) for every reviewer spawned in this round — the script also adds `agent-reviews/` to the repo's ignore list (see `docs/design-decisions.md` §12 for the append's duplicate-tolerance). When spawning any reviewer that carries the file-based-output contract (`Write` tool plus a `### File-based output` section — see `agent-review` checklist item 15), pass `findings_path: agent-reviews/<agent-name>-<suffix>.md` in the prompt, where `<agent-name>` is the agent's name.

A project-layer reviewer that lacks the contract must not receive a `findings_path` — passing one anyway re-arms the heredoc-abort-on-large-findings failure the contract exists to prevent. Spawn synchronously (not `run_in_background`) — the mandatory read-back step must execute in the same turn. After the reviewer returns: if it reported a successful write, `Read` the findings file, starting with `## Recommendations` and reading the full file when the count is non-zero. If it reported a write failure, use the inline findings it fell back to instead. See `docs/design-decisions.md` §12 for the mechanism, its measurements, and rationale.
...
When the plan carries an assumption ledger (see `plan-it` Step 5) and this is a re-review of a prior round, include this instruction in every spawned reviewer's prompt: diff the current plan revision against every `[verified]` and `[engineer-verified]` ledger row for continued consistency. Flag a contradiction of an `[engineer-verified: "<quote>"]` row's quoted content to the human; do not resolve it, mirroring the tag's own rule for the plan author. Exception: for a bare `[engineer-verified]` row with no quote, treat the whole claim as protected. Otherwise, a contradiction with the rest of the row's claim is an ordinary B5 finding. If the revision touches a row already confirmed in a prior round, the reviewer's findings must name it under **Previously-settled, now reopened** — surfacing the human's own version of the failure (re-litigating something already decided), not just the agent's.

Handle an `[author-inferred]` row on every review round: it is the plan's own reasoning, so resolve it directly on your own evidence like `[unverified]` and never escalate it to the human. When a finding's load-bearing premise is an `[author-inferred]` row, say so in the finding rather than building on it as established. A row tagged `[author-inferred]` whose claim you cannot resolve from repo evidence is itself a finding, because the tag is then the wrong one.
