...
**Neither a named-tool carve-out nor a structural-tool-shape test. Key the exception on the act of authorization: the owner authorizes one figure, from one command, into one artifact.** This lands at the same practical destination the session leaned toward — general wording, no `review-round-cost --pooled` by name, a future tool qualifies with no further doc PR — but it replaces the proposed hinge, which does not survive contact with the repo.
...
New subsection, inserted between current lines 167 and 169 of `docs/private-project-redaction.md`. Wrap to the file's existing ~68-column width. The implementer may tighten wording; the three-item exclusion list and the one-figure/one-artifact sentence are invariants, not stylistic choices.

```markdown
### The owner can authorize one figure, case by case

The bar above holds by default: a wider read stays with the owner and
publishes nothing. The owner can release it for one specific figure.
Propose the figure, the exact command that produced it, and the
artifact it would appear in; publish only after an explicit,
in-session yes, and cite that authorization beside the figure. A yes
recalled from any other transcript or artifact does not count — only a
live answer in the current session does, the same standard "New
figures against the grandfathered set" below already sets. The command
is cited so a reader can re-run the measurement. Unlike the
single-account case above, it guarantees nothing about scope — the
authorization is the whole control, which is why it is granted per
figure, why it leaves a trace, and why the mechanical backstop behind
it is this repository's own human-only merge gate: the owner reviews
the PR before it merges, and a citation for a yes never given is
exactly what that read catches.
...
re-run over a grown corpus is a fresh ask. Before citing an authorized
figure alongside a rate or count already published elsewhere, name
that composition in the proposal — "New figures against the
grandfathered set" below is the check that applies.

An authorization releases the corpus-scope bar and nothing else. Three
...
privately and publishes nothing without that authorization, cited, for
that one figure. If in doubt, don't.
```

The citation now names the authorization exception's limits, not only
"the two exemptions" — ciso-reviewer's finding: an agent reasoning from
CLAUDE.md alone, holding a genuine yes for one figure, had no textual
signal that the account-cardinality and `--share-only` bars sit outside
the exception's reach, since neither appears in CLAUDE.md and the old
citation didn't claim to point at them either.
...
`.claude/skills/code-review-claude-config/SKILL.md` — extend the existing sub-bullet's `unless` clause:
...
- a figure computed over a corpus wider than this repository on one
  ...
  or the artifact cites the owner's authorization for that exact figure
```

The clause reads as exhaustive today, so a reviewer trusting the checklist would P1 a validly authorized figure. Both escapes are diff-visible, which is what the checklist wants. The `--share-only`, account-cardinality, and auto-publishing-scope sub-bullets stay byte-identical — the new subsection's exclusion list is written so those three keep agreeing with the doc by construction rather than by luck.
...
- **`.claude/skills/code-review-claude-config/SKILL.md`** (modify, the wider-corpus sub-bullet only) — extend its `unless` clause as given. This is a `SKILL.md`, so `/skill-review` is **hook-enforced**: `require-skill-review.sh` blocks `git commit` until the behavioral-equivalence marker is written (`.claude/rules/review-pipeline-dispatch.md`). Leave the other six sub-bullets and the existing `` § "Publishing a tooling measurement" `` citation byte-identical, so `test_tooling_measurement_code_review_skill_citation_is_still_present` stays green.
...
4. **`/skill-review`** on the `code-review-claude-config/SKILL.md` diff — hook-enforced, blocks the commit until its marker is written. Per `.claude/rules/skill-and-agent-self-review.md`, re-run a fresh allow/deny fixture pair against the amended sub-bullet: a must-flag fixture (a wider-corpus figure with no authorization citation) and a must-allow fixture (the same figure with one).
...
- **Reinstating any machinery PR #1011 deleted** — the "What it permits" scope lists, three standing bars, permitted-split conditions, and time-series ban. The "a re-run over a grown corpus is a fresh ask" sentence is deliberately doing the repetition work in one line; it is not the time-series ban returning in smaller form.
...
- **Any relaxation of `--share-only`, account cardinality, or the per-project/per-account/per-engagement dimension bar.** The new subsection's exclusion list exists to foreclose all three.
