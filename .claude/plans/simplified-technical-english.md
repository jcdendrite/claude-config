...
**`tighten-prose`** — verb-first like other invoke-only workflow utilities,
an accurate functional name, and it now matches the `## Prose tightening
pass` section name already chosen in `/pr-description`. ASD-STE100 stays in
...
- `claude/.claude/skills/tighten-prose/SKILL.md` (new) — invoke-only, no
  ...
  ---
  name: tighten-prose
  description: Rewrite prose for clarity and concision — short sentences, active voice, one idea per sentence, consistent terminology — without dropping or softening any fact, number, decision, or hedge. Operates on a file path or on literal input text. Dispatched by /pr-description's prose-tightening pass; also invocable standalone any time text needs the same treatment.
  ---
  ```
  Body, under the 200-line `check-skill-length.sh` cap (default tier, no
  exception needed):
  1. **Preserve every fact and its original strength — stated first, as the
  ...
  2. **Carve-outs, left untouched verbatim:**
     - Syntactic: fenced code blocks, inline code spans, file paths,
       identifiers, proper nouns (tool/library/repo names), numbers and
       units, markdown headings, any machine-managed delimited block (e.g.
       `<!-- pr-cost:start -->`/`:end`).
     - Semantic (added after the Opus review — these are undetectable by
       pattern-matching, so they're excluded rather than trusted to a
       rewrite-then-check loop): hedges and modal verbs ("may," "should,"
     ...
     - Whole-sentence-class: deploy/coordination steps, security-invariant
       ...
       than re-deciding it.)
  3. **Rewrite rules** for everything outside the carve-outs above, each
  ...
  4. **Input handling.** If given a file path, `Read` it, apply the
  ...
  5. **Self-check before returning.** Re-read input and output side by
...
- `docs/skills.md` (modify) — add the skill to the main skills list and to
  ...
  category (alongside `brief`, `handoff`, `pr-description`, etc.) — bumping
  that category's count from seven to eight and the table's total from
  twelve to thirteen.
...
- Manual: draft a PR body from a deliberately verbose/passive-voice
  ...
  paragraph to confirm it works outside `pr-description` too.
