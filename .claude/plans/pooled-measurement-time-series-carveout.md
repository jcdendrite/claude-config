...
| 38 | Between rows 37 and this row, `origin/main` advanced with an independently-developed, already-merged sibling PR (#961, "Scope multi-account/machine pooling to dimensionless shares only") that inserted a new "Account and machine scope" paragraph at the same seam this branch amends — permitting a dimensionless share (only) to span multiple `CLAUDE_CONFIG_DIR` accounts/machines, where every other reporting mode stays single-account. A rebase onto that commit auto-merged with no textual conflict, but a fresh `ciso-reviewer` pass dispatched specifically to check whether the two independently-reviewed amendments compose safely found a genuine, unaddressed `[BLOCKER]`: this branch's split sub-bullet permits a share as one of the "otherwise permitted" statistic forms, and nothing in either amendment excluded a before/after split of a *cross-account* share — composing the two recovers exactly the subtraction-attribution attack that PR #961 itself describes (bracketing a cross-account share around a named, datable pivot lets one account's activity be subtracted out to attribute the shift to the other), which neither a same-account split nor a non-split cross-account share supplies alone. Both amendments' own review rounds ran against document states that predated the other's text, so neither review pass had the composed result in front of it. Fixed by choosing the reviewer's "forbid composition" resolution over "permit with added conditions": the split sub-bullet now requires both sides to resolve to a single account and machine, stating the cross-account share exception does not extend to a split. This also mooted the reviewer's secondary `[CONCERN]` (the pivot-naming ordering check's implicit single-transcript scope), which only mattered if a cross-account statistic were split-eligible at all. | `[verified: ciso-reviewer, seam-interaction pass, agent-reviews/ciso-reviewer-1789109727-pooled-measurement-t-seam.md]` |
| 39 | A `comment-discipline-reviewer` pass on row 38's fix found the new sentence restated the subtraction-attack mechanism the "Account and machine scope" paragraph already states in full, and chained that restated rationale onto the mechanism clause via a comma. Fixed by trimming to one clause cross-referencing "Account and machine scope" as the canonical rationale, rather than re-deriving it — the same single-source-of-truth discipline rows 30 and 35 already applied elsewhere in this section. A final `ciso-reviewer` verification pass then confirmed the branch clean, including checking two residual readings the fix itself could have left open (each side individually single-account but to *different* accounts; a separately-published non-split cross-account share paired with a same-account split) and finding both already foreclosed by existing text ("exhaustively partition" requires one fixed pool; a non-split cross-account figure carries no temporal axis to pair with a pivot). | `[verified: comment-discipline-reviewer, agent-reviews/comment-discipline-reviewer-1789110021-pooled-measurement-t-seamfix.md; ciso-reviewer, final verification, agent-reviews/ciso-reviewer-1789110021-pooled-measurement-t-seamfix.md — verdict: clean, BLOCKER resolved]` |
