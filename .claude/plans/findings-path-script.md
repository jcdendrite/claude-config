...
4. **Slug.** `git rev-parse --abbrev-ref HEAD` piped through `tr '/' '-'` then `cut -c1-20`, in that order. Detached HEAD yields the slug `HEAD` and is not special-cased, matching today's recipe. The slug is a human-readable hint, not an identifier.
