...
18. Deny rules union rather than override across the two lower-precedence settings-file layers actually tested — project-scope and project-local — so no `permissions.allow` entry or plain omission at either layer un-denies a user-scope entry. The opt-out is therefore editing the tracked file — the tradeoff §43 says every key in that file already carries. `[verified: pre-implementation gate check 7]` A project-scope `permissions.allow` entry, a project-local `permissions.allow` entry, and plain omission were each tried in turn against a live user-scope deny; `ScheduleWakeup` stayed absent in all three. The command-line flag layer, which §43 places above both tested layers, was not tried. This claim also sets the precedent future deny entries in that table are read against — including that untested-layer caveat, not just the tested-layer result.
