...
file and from §44 beyond the narrow exceptions G5 names. The corpus mixes
...
**Why nothing addresses `noop` directly.** The misconception has no expression surface once the tool is absent from context — the model cannot construct a call to a tool it cannot see. A `CLAUDE.md` line or skill clause warning against a tool the deny already removed is the compounding-defensive-layer shape §41 and §42 both name as a wrong-foundation tell, and it would spend budget `check-claude-md-length.sh` polices for zero behavior change. The `noop` finding earns exactly two homes: §44's narrative (so the mechanism is recoverable if the deny is ever removed) and the upstream report (where a schema field the model reads as a mode is actionable). Neither is a runtime mechanism.
...
- `CLAUDE.md` is the only surface here that loads into every session — the four shipped mitigations (`docs/skills.md`, `docs/auto-mode.md`, §44, `CHANGELOG.md`) are all pull-based, and none reaches a consumer who types `/loop <prompt>` and watches it stop.
...
- `docs/skills.md`'s `/loop` row is where a consumer checks what `/loop` currently does; §44's Revisit list (below) carries the reopening condition if a consumer reports the silent truncation.
...
5. The self-paced dynamic `/loop` mode degrades under the deny to a small number of iterations at the harness's ~20-minute fallback cadence before ending silently; fixed-interval `/loop <interval> <prompt>` is unaffected because it routes through `CronCreate`. `[unverified]` — E2 labels the iteration-count consequence an inference from the scheduled-tasks docs, not a documented behavior. The pre-implementation gate below converts this to a verified observation before anything ships.
6. §41's third Revisit condition is met: the well-formed non-loop call is the *majority* sub-mode rather than the minority §41 assumed, and the population where a wakeup plausibly fired sits at large context sizes where a re-invocation is a real cost. `[verified: E3's structural classifier, resolving tool_result → tool_use_id → tool_use.name]` — the supporting counts and context medians exist but are unpublishable under G5, so §44 states this ordinally and reproduces no figure.
...
9. The attribution of the plausibly-fired subset is a magnitude, not a logged fact — no harness log distinguishes a fired wakeup from a task-completion notification. `[unverified]` — §44 must not present it as a count either way, which G5 independently forces.
...
18. Deny rules across settings scopes union rather than override, so no lower- or higher-precedence file un-denies a user-scope entry. The opt-out is therefore editing the tracked file — the tradeoff §43 says every key in that file already carries. `[unverified]` **Pre-implementation gate check 7 resolves this.** The `CHANGELOG.md` and `docs/auto-mode.md` wording is gated on its result. Do not state "editing the tracked file is the only opt-out" as fact before check 7 passes. If a lower-precedence file does re-enable the tool, name that path as the opt-out instead. This claim also sets the precedent future deny entries in that table are read against — shipping it untested would propagate further than this one row.
...
21. The engineer has not yet been told E4's verdict and has not accepted the loss of self-paced `/loop`. `[engineer-verified]` — see **Open confirmation** below; this gates implementation.
...
| A new §44 recording the reversal, the mechanism-exhaustion lesson, and the `noop` account | Makes the reversal re-decidable and preserves the reason the deny exists if it is ever removed; states E3's bearing ordinally with no figure reproduced. | root; row 6; row 10; G5 |
...
**Open confirmation — the plan cannot proceed past the pre-implementation gate without it.** The engineer asked for the research that produced E4 and has not been told its verdict or accepted the cost. Present, in one turn:
...
- The pre-implementation gate's *observed* answer for what a bare `/loop <prompt>` does under the deny (not row 5's inference).
- That fixed-interval `/loop <interval> <prompt>` survives intact.
- That the entry ships to every stow consumer with a tracked-file edit as the opt-out (row 18).
...
- §44's accuracy depends on E3's disposition analysis and E4's verdict, both of which live only in the dispatching session's context — restating them in a dispatch prompt is `plan-it` Step 5's own named do-not-split condition.

## Critical files

- **`claude/.claude/settings.json`** — append `"ScheduleWakeup"` to `permissions.deny`, immediately after `"EnterPlanMode"` (line 66), keeping the two bare tool-name entries adjacent and last in the list. Change nothing else in this file; in particular do not touch `skillOverrides.loop` (still `"name-only"` at line 90) or `model`. **Reuse:** the `EnterPlanMode` entry's placement and bare-name form.
...
- **`docs/design-decisions.md`** — two edits:
  - Insert one status line directly under the §41 heading (line 679): that §44 supersedes its conclusion because its mechanism survey did not reach the settings layer, and that its three rejected mechanisms stand as recorded. Do not rewrite §41's body, its Revisit list, or its Sources.
  - Append `## 44. <title> (2026-09-03)` after §43's last Sources bullet. Content contract — one element each:
    ...
    - The blast-radius statement: every stow consumer, self-paced `/loop` degraded, fixed-interval unaffected, opt-out is a tracked-file edit (rows 5, 18, 19).
  ...
  - §44's **Revisit** conditions to write: the model substitutes a worse wait mechanism for the removed tool (repeated `ListAgents`/`TaskOutput` polling, a `Bash sleep`, or a `CronCreate` call — E3 measured zero `CronCreate` corpus-wide, so any appearance is signal); a genuine `/loop` need arises in this repo's pipeline, which E4 found none of; **a stow consumer reports a silently-truncated self-paced `/loop`** — the one failure mode with no instrumentation at all, since row 19 concedes consumer usage is unobservable and the degradation surfaces no error; or `ScheduleWakeup` gains its own out-of-`/loop` validation upstream, which moots the entry.
  - §44 must also state that **reversal is not friction-free**: removing the line re-triggers `ask-review-permissions.sh`'s `ask` decision and a `review-permissions` re-review, exactly like the forward edit (row 12). "One line to revert" is the diff size, not the process cost.
- **`README.md`** — widen the `permissions.deny` description at **line 247** ("a `permissions.deny` hard floor for `sudo` and secret-file reads") and **line 438** ("hard-blocking `sudo` and well-known secret-file reads") to also name tool-availability entries, pointing at `docs/auto-mode.md` for the list. Both phrases predate `EnterPlanMode` and are already inaccurate for it, so this closes existing drift rather than only describing the new entry; it is a one-line accuracy fix in a file this PR's own table edit summarizes, not scope creep. **Reuse:** the existing parenthetical cross-reference to the Auto mode section already on line 247.
- **`docs/skills.md`** — amend the `/loop` row in the name-only table (line 82) with the capability caveat: fixed-interval invocation works, the self-paced no-interval mode is degraded because `ScheduleWakeup` is denied, with a pointer to §44. Leave §17 and the surrounding budget prose unedited.
...
- **`.claude/plans/prevent-non-loop-schedulewakeup-calls.md`** — this plan, committed per `plan-it` Step 7. Subject to the same G5 redaction constraints as §44: it ships in the same PR.
...
1. **Pre-implementation gate — run before the settings edit, and before asking the engineer to confirm.** Follow `.claude/plans/plan-mode-workflow-discipline.md` § "Pre-implementation gate — run, passed" for setup shape: a **full copy** of the stowed settings file (not a minimal one), a **control tool** added to the same deny list so a negative result is attributable, and **tool calls demanded rather than a tool inventory** since self-reported tool lists are unreliable.
   ...
   **Run the gate at user scope as primary.** The precedent gate ran in a scratch *project*-scope `.claude/settings.json`, but stow lands this entry at *user* scope (`~/.claude/settings.json`) — that is the scope it actually ships to, so it is the scope checks 1–5 must be observed at. Run the project-scope copy afterwards as a comparison only. Do not invert this: spot-checking one behavior at the deployment scope and four at a scope the entry never occupies leaves the four unverified where it counts.
   ...
   4. A bare `/loop <prompt>` (self-paced) must be run and its actual iteration behavior observed and recorded — this is what converts row 5 from inference to fact.
...
- **A `PreToolUse` gate, a `PostToolUse` advisory nudge, and a `CLAUDE.md` line.** All three rejected in §41 on grounds that still hold, and two of the three now also fail row 20's prevent-not-reject bar. Named in §44 rather than merely omitted, so a future session does not re-derive them.
...
- **Re-deriving E3's figures from a public-only corpus to make them publishable.** Real work with no consumer here — §44 states its case ordinally.
