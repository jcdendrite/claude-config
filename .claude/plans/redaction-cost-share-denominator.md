...
- **M1 does not rescue PR 927's headline figure**, contrary to this session's first reading. `30.3%` is `round $ ÷ branch $` — numerator and denominator are both branch/round constructs, so the unit problem sits one level below the reporting-form axis M1 amends. Permitting the share form is orthogonal to it.
