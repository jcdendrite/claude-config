...
- **"in Durable text, below" or "in Durable text below" at :100.** Costs +1 or +2 bytes, so net-zero would need a compensating trim elsewhere. It also keeps a positional cue, and a positional reference is what leaves the pointer ambiguous today.
...
|---|---|---|
| :100 "in the section below" → "in §Durable text" | A name survives both moves and heading-level changes. Position-relative wording depends on where the section sits. | `anchors: root, row1, row6, row7, row8` |
...
| Record: delete the two follow-up bullets at :128-129 that quote "the section below" and "This section governs" | The follow-up list describes current state (Axis 3), and these two items are now done. | `anchors: row3, row4, row5` |
| PR body notes that the record's re-review trigger fired and no residual changed | The record's re-review line (:110, "Any later change to CLAUDE.md Agent Core reopens this section") says any change to Agent Core reopens that section. | `anchors: row12` |
| No new test, no CHANGELOG entry | No rule's content changes. A phrase pin would be a proxy for the text itself, and no test pins "the section below" or "This section governs". | `anchors: row10, row14` |
...
| 3 | The record's follow-up list holds both phrases as open items at :128-129, below the "Merge stays human-only" bullet (:126) and the "the rules above" GH-1091 bullet (:127). | `[verified: record :124-129]` |
| 4 | Under Axis 3's decision test, the record's follow-up list is editable current state: it "lists follow-ups that are still open". | `[verified: .claude/plans/record-followups.md:20-24; CLAUDE.md:75-82]` |
...
| 10 | No test pins either phrase or CLAUDE.md's byte count. `test_global_claude_md_groups.py` pins only the heading's placement. `test_output_preferences_layering.py` reads Prose's bold lead-ins only. A `*.py` grep for "section below", "This section governs", "further constraints" and "durable docs only" finds no CLAUDE.md pin. | `[verified: grep; test_global_claude_md_groups.py:111,248; test_output_preferences_layering.py:91-93]` |
...
| 13 | The only other files that quote "the section below" or "This section governs" are earlier plans (`output-format-best-practices.md:70`, `trim-comment-verbosity.md:83`, `claude-md-agent-core.md`, `record-followups.md`). They are preserved plan records and are not edited. | `[verified: repo grep]` |
...
| 16 | The net-zero-or-fewer target and the exclusions (the opening line, "Merge stays human-only", "the rules above") have no primary source in the repo, and no engineer statement backs them. | `[unverified: no primary source]` |
...
- `docs/design-decisions/global-claude-md-agent-core-and-main-session-groups.md`. Delete the two follow-up bullets at :128 and :129. Keep the lead-in (:124) and the two remaining bullets (:126-127) unchanged.
...
3. Read the record's follow-up list (near :122-128). The lead-in is followed only by the "Merge stays human-only" and "the rules above … GH-1091" bullets, and `## Byte margin` follows.
