...
Confirm `select-tests.py` selected the skills domain for Phase A and the transcript-analysis domain for Phase B before trusting a green result. GH-882 (a prior under-collection bug that dropped a domain directory when that directory and a file inside it were both selected) is already fixed as of `ea64d3ee` (#891), an ancestor of this branch — this confirmation step is a sanity check on the fix holding, not a live workaround.
