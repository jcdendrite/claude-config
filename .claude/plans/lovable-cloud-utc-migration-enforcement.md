...
- `plugins/lovable-cloud/skills/lovable-cloud-migration-author/SKILL.md` — short instruction surface, **model-invocable** (a trigger description that fires when authoring a Supabase migration; not `disable-model-invocation`): run `new-migration <slug>` and write the printed filename. (This is what makes Claude *use* the generator instead of hitting the deny wall on the first try; the hook is enforcement, the skill is awareness.) Adding a SKILL.md makes `/skill-review` a hook-enforced pre-commit gate — run it.
