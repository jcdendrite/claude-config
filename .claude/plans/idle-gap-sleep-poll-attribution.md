...
Two known limits ship as disclosed report prose rather than as more matching machinery. The match is textual with no shell parsing, so a quoted or heredoc-embedded `sleep` counts, over-counting sleep-poll waits. Requiring a leading digit means `sleep $INTERVAL` falls to `other Bash wait`, under-counting sleep-poll waits. Both are named in the block's prose with their direction.
