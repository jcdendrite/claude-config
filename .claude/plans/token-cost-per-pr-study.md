...
| A7 | `gh pr list --state merged --json number,headRefName,additions,deletions,changedFiles,mergedAt` returns every field and `headRefName` survives merge; `gh pr view <n> --json commits,reviews` yields commit and review counts (`commits`/`files` are `pr view` fields, not `pr list` fields). | [verified: `gh pr list --state merged --limit 5 --json number,headRefName,additions,deletions,changedFiles,mergedAt` and `gh pr view 681 --json commits,reviews`, both run 2026-08-17 after the API recovered — all fields returned, `headRefName` non-empty on all 5 sampled merged PRs.] |
...
| A10 | ~17% of in-window spend is attributable to no PR branch: named feature branches ≈83.3%, `main` 15.9%, `HEAD` 0.9%. Unbranched spend is within the live-corpus noise floor; no exact figure certified. | [verified: `cost --this-repo --since 30d` unfiltered vs. `--branches` runs, 2026-08-17] |
| A11 | Window totals fluctuate between successive runs as the corpus grows live. Context class 87.4% (cache read 57.9%, write-1h 18.7%, write-5m 10.8%); output 12.6%, input 0.1%. Main 69.3%, subagent 30.7%. | [verified: `cost --this-repo --since 30d`, 2026-08-17; redaction active, no `STALE PRICING` banner. **Provenance:** these are this repo's own spend only — `--this-repo` resolves to `claude-config` worktree project dirs by identity; a single-account `--summary` run over the same window scanned the same 208 transcripts, with the gap attributable to live corpus growth between runs. No other account's or client's spend is included in any figure in this plan.] |
