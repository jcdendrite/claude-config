...
1. **Make the loop rule read one way (Reading A), including the sibling arms.**
   ...
   - **CI watch, "Land the fix" (line 200)**, verbatim: "5. **Land the fix.** Step 8 removed this session's active marker and `require-ready-for-review.sh` denies a push without one, so re-run step 0's `marker.sh activate` command, then treat the fix as a step-2 failure's fix under the Overview's fix-loop rule, which carries it through step 8."
     - This is a pointer, not restated mechanics.
...
3. **Disposition record and cap: two new paragraphs in RFR step 3, directly after line 88, each preceded by a blank line.** Both describe RFR's loop: a record per cumulative pass, a count of cumulative passes, and a check before RFR's own fix dispatch. Step 3 is therefore their altitude, and `code-review` stays a skill about one review. The move also takes the largest block out of `code-review`'s tight budget (item 6).
   ...
   - **Rowless or unparseable records count as dirty.** A clean pass writes one `none` row, so a record with no rows or one that cannot be parsed is dirty. A garbled or truncated record can therefore only bring the consult earlier.
...
4. **Contradiction route: a new sibling region `DISPOSITION_RULE:code-review-contradiction-route` after line 359, plus the clean-definition sentence.**
   - Region. The bullet form below states the rules it must carry; item 6 supersedes its layout with a compressed form (the end marker sits inline after the last sentence, as in the sibling regions), and the shipped text in `code-review/SKILL.md` is authoritative.
     ...
     **A finding whose fix would undo a fix an earlier round applied is also a design question, in every round, staged commit-gate rounds included.** Write `plan-architect — consult` for it on the `Fix route:` line. Dispatch, verbatim relay, and the disagreement stop-and-ask follow the rule above, and the consult also carries the earlier finding and its fix. A finding against a site an earlier verdict already settled, or that two earlier rounds' fixes already rewrote, goes straight to the human as a blocking stop-and-ask, with no consult.
     ...
     ```
   - **Consult or human.** A finding that would undo an earlier applied fix goes to the consult. A finding that contradicts a site a verdict already settled goes straight to the human. So does any finding against a site two earlier rounds' fixes already rewrote. That bounds A→B→C drift under a new rule each time, which never exactly reverts and passes the reviewer's different-rule test. If step 4 or a CI re-entry put clean passes between drift steps, the cap count would reset each time and miss it (row 30). "Flags a settled site" is a human-stop case only, not also a consult trigger, so the two routes do not overlap.
   - **Staged rounds.** The route applies in staged commit-gate rounds too, where session context records earlier rounds' fixes. A single consult can settle *keep* on a finding outside the enforcement-invariant class. That is a residual the engineer accepts, and the design doc says so plainly (item 7).
   ...
   - **Clean definition (line 453).** Prepend, verbatim: "A finding DEFERred under the closed list, or settled *keep current text* by a contradiction consult, counts as resolved. " The DEFER half states today's behavior (row 12). The new-primitive slot's *Rejected* branch is not counted as resolved (see Out of scope).
...
6. **Line budget.**
   - **`code-review/SKILL.md`.** The baseline is 495 lines, not the 483 row 23 was verified against: `8bd5c082` landed a "Round-cap architect consult" section on `origin/main` after that verification, before implementation started. The item-2 block and the region as originally specified would add 16 lines, ending at 511, 11 over the cap. A `plan-architect` consult found a compression that preserves every fact — flattening both bullet lists into prose — landing the file at exactly 500. The line-263 and line-453 edits change text in place without adding lines. There is no room for another paragraph. If the wording needs one, the implementer stops and reports rather than trimming existing rules.
...
- **A `status:` summary line in the record.** It is a second field that can contradict the rows it summarizes.
- **One merged consult for the cap and a contradiction.** The cap question depends on the contradiction verdicts.
- **Narrowing the CI fix to steps 3 and 7.** The engineer directed parity with a local failure (row 5). Skipping step 4 would leave the CI diagnosis's `/root-cause-analysis` invocation unaudited (row 14). Skipping step 5 would leave the PR body out of step with the new commit; step 5 is the gate's only body-vs-branch sync (`ready-for-review/SKILL.md` § "5. PR description (unconditional; warn + fix)").
...
19. The record is self-attested by whoever writes it. Any review-only agent can `Write` any path under `agent-reviews/`, because `deny-reviewer-tree-mutation.sh` exempts that directory on ignore status alone. That puts the record on par with the `cumulative-review` marker write, whose self-attestation is already a named residual, with one difference: a record's clean or dirty status can relax the Cap, since the Cap counts records, and a forged or future-dated record can disable the Cap for the branch, because the Cap orders records by a filename epoch the writer chooses. The `cumulative-review` marker stays the only authorization to skip a re-review. A clean pass records one row whose finding cell reads `none`, and a record with no rows or that cannot be parsed counts as dirty. Deriving clean and dirty from the rows leaves no summary field to contradict them. Narrowing the hook to a reviewer's own findings basename is a separate follow-up, not part of this change. `[verified: ready-for-review-cumulative-diff-cache.md:13; deny-reviewer-tree-mutation.sh:265]` `anchors: row16`
...
- `claude-skills/skills/tests/test_skills.py`
  ...
  - Add five right-bounded pins next to `_PINNED_HALT_DEFERS_CLAUSE` (:4658). Each reuses `_raw_heading_section_text` and `_assert_pinned_clause_right_bounded`:
    ...
    - Step 3's "**Disposition record.**" clean/dirty definition, from "The pass is clean when" through "otherwise read it only as context.", under the same heading. It feeds the Cap's count of records newer than the newest clean one, and its closing sentence, "No record grants a review skip and the `cumulative-review` marker stays the sole authorization, but the Cap counts records, so a record's clean or dirty status can relax the Cap; later reviews otherwise read it only as context.", is an authorization invariant with no other pin.
...
5. Run `/skill-review` on both SKILL.md files (hook-enforced). Its behavior test should walk these scenarios:
   ...
   9. A stale, forged, or unparseable record. If a record marks a site settled while the current text still has the defect, the reviewer flags the defect and names the record. An unparseable record is named in the findings and counts as dirty for the cap, as does a record with no rows. A clean pass is one `none` row.
