...
- **M6 — Correct the stale "main has no lever" claim in all four arms it
  ...
  names. Fix both source-code arms (`transcript-analysis.py:7031-7033` and
  `:7084-7087`); no committed test pins the printed strings (row 17). Once the
  printed strings change, the captured sample block at
  `docs/transcript-analysis.md:815-937` —
...
| 9 | Both narrative-prose sites in `docs/transcript-analysis.md` are outside captured tool output and are therefore freely editable: `:974` sits in prose about the switch-delta section, `:964` in prose about the cause-attribution table. The captured sample is a single fenced block running `:815-937`, and it reproduces the stale claim twice, at `:872-874` and at `:916-919`. | `[verified: read of docs/transcript-analysis.md:860-990; fence boundaries confirmed by scanning the file's fence lines]` | M6 |
