...
- `cmd_rearm_backtest` (transcript-analysis.py:10531) wraps `_rearm_backtest_report` (10543).
- Scopes sessions via `--this-repo`/`--projects`, `--since`, `--branches`.
- Extracts and prices each session's real turn sequence (`_extract_rearm_session_turns`, 10139).
- Builds the counterfactual post-crossing repricing ramp curve (`_ramp_curve_from_corpus`, 10227).
- Joins `.handoff-nudge.log` for the real (observed) operator-response lag (`_operator_response_lag_from_log`, 10363), excluding `action=block` lines (10397) since a forced overshoot isn't voluntary compliance.
- Simulates each candidate spacing under perfect- and lagged-compliance models (`_simulate_rearm_spacing`, 10413), reporting `$` delta and mean-context (`C_bar`) delta per spacing vs. a no-rearm baseline.
...
- **Consumers of the binary distinction that would need updating, not just tolerate a new value:**
  - `_operator_response_lag_from_log` (transcript-analysis.py:10397) explicitly checks the literal string `"block"` — a third tier is not automatically excluded from the lag measurement unless this check is revisited.
  - `_parse_nudge_log_entries` (10299-10360) already treats `action` as optional-if-present (10344-10345), so it would parse a new value without a code change, but every reader must still decide what to do with it.
  - `spend-over-threshold` and `rearm-backtest`'s simulation never read `action` at all — unaffected.
  - Test files with literal-string assertions on `action=block`: `test_transcript_analysis.py` (~18126-18353) and `test_nudge_handoff_near_context_cap.py` (multiple sites, ~1348-2550).
...
**D. Conversion signal, root resolution, phrasing history, and cross-machine mechanics:**
...
- **The exclusion count is not a measure of the multi-root defect.** `_operator_response_lag_from_log` (`:10394-10409`) excludes entries whose session is absent from `session_traces`; more roots make that set larger, so multi-root can only reduce exclusions. The real defect is the asymmetry: most declared roots' sessions feed spend and traces while their nudges sit in unread logs.
- **Missing per-root logs are already safe.** `_read_bounded_log_lines` returns `[]` for an absent or unreadable file (`:8740-8741`, `:8747-8748`), so the root-aware join needs no guard for the roots with no log. Its `_NUDGE_LOG_MAX_READ` tail-truncation (`:8743-8745`) now applies per root rather than to one combined read.
...
- **Multi-account aggregates are established practice in this repo.** Precedent:
  - `docs/handoff-nudge.md:26` publishes "426 fired sessions across every declared account."
  - `docs/case-studies/handoff-threshold-impact.md:120-121` treats its corpus as spanning every configured account for its retention-floor and as-of-window facts.
  - `docs/case-studies/cold-cache-attribution.md:268` treats the multi-account corpus as its default.
  - `docs/case-studies/hashline-edit-format.md:93-104` publishes a per-account breakdown under `account-N` labels, "never the raw config-dir path or account name."
  - `docs/cost-levers-considered.md:37-39` names account types.

  `cost --summary`'s narrowing (`scope.py:513-524`) is an attribution guard for a *labeled* per-account figure, not a bar on pooled aggregates.
...
The engineer works across two machines, so a single-machine corpus is half the available evidence. The nudge's *phrasing* was never examined as a lever. And the adherence rate that would tell levers 2 and 3 apart is retrospectively measurable from a log line this repo already writes and already parses but has never read.

**The measurement this plan specifies reads the wrong population.** `_resolve_cost_roots` unions the default config dir with every entry in `~/.claude/transcript-config-dirs` (`transcript_analysis/scope.py:526-535`), so `rearm-backtest --this-repo` is multi-root by default with no flag given — several roots on this machine. But `_rearm_backtest_report` joins its nudge log at a single hardcoded path, `config_dir() / ".handoff-nudge.log"` (`:10627`). Sessions from every declared root feed `sessions_data`, `session_traces`, and the dollar simulation, while only one root's nudges are ever joinable. The median operator-response lag — the sole input to the realistic-compliance arm and to Gate C — is therefore measured on one account and applied to every declared root. The same defect would hit the conversion instrument, whose numerator would come from one log while its denominator spanned every declared root, biasing the conversion rate downward by construction.

**So the fix comes before the measurement, and it widens rather than narrows.** Multi-root scope is kept: pooled, pseudonymous multi-account aggregates are this repo's established practice for exactly this kind of operational telemetry. Precedent:
- `docs/handoff-nudge.md:26` publishes "426 fired sessions across every declared account."
- `docs/case-studies/hashline-edit-format.md:93-104` publishes a per-account breakdown under `account-N` labels.
- `docs/case-studies/cold-cache-attribution.md:268` treats the multi-account corpus as its default.

What changes is the join: read each resolved root's own `.handoff-nudge.log` and join within-root. That fixes the bias, raises the lag sample and the conversion denominator instead of shrinking them, and serves both instruments in one function.
...
**One discarded run, disclosed not pasted.** A peer session on the macOS machine already ran the pre-correction invocation and returned a report resolving multiple roots across 118 project dirs. It is unusable for any gate — its lag median was computed from one root's log while its sessions spanned every declared root, which is the defect this plan fixes. Record that it happened and why it was discarded; paste none of its figures, since a number produced by a known-biased join would otherwise sit in the plan file looking like evidence.
...
**Root:** The re-arm spacing was tuned against a corpus truncated near 230,000 tokens. The deeper tail under `BLOCK_AT=470,000` may have moved the operator-response lag that spacing was grounded on. Two further named levers — a two-tier nudge and the nudge's own phrasing — have never been priced. The adherence rate that would distinguish them has never been read, despite being logged and parsed. The instrument that would answer all of this joins its log against one account while scanning every declared root.
...
| 4a | **`--this-repo` is multi-root by default.** A populated `~/.claude/transcript-config-dirs` makes `rearm-backtest` scan every declared root with no `--config-dir` given; this machine declares several. Omitting the flag is not single-root — it is the opposite. | `[verified: transcript_analysis/scope.py:363-367, :526-535; the discarded macOS run's own header confirmed multi-root resolution]` |
| 4b | **The report joins its nudge log at one hardcoded path while scanning every root**, so the median lag is measured on one account and applied to every declared root. This is the defect the plan fixes, and it biases the realistic-compliance arm and Gate C. | `[verified: transcript-analysis.py:10627 vs. the multi-root `session_traces` built at :10619]` |
...
13. **A root with no log file** contributes zero entries and does not raise, exercising `_read_bounded_log_lines`' absent-file path against this machine's real mixed has-log/no-log shape (row 21b).
...
- **Investigating why a few declared roots have no `.handoff-nudge.log`.** Recorded and disclosed (row 21b); whether those accounts never fired a nudge or never ran the hook is an operational question this plan does not need answered, since either way their sessions correctly stay out of the fired-session denominator.
...
- **A new `docs/case-studies/` file.** The deliverable is four verdicts, one instrument fix, and two recorded runs — register-sized, matching the `:472-491` precedent that also built its own instrument. A case study is the format for a before/after cost comparison, which this plan does not run. Addendum: [`case-studies/rearm-spacing-deep-tail.md`](../../docs/case-studies/rearm-spacing-deep-tail.md) was added anyway, with its own separate owner approval to republish this plan's figures there — see that file's Sources section.
