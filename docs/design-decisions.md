...
`/plan-review` (`plan-review/ROUTING.md`) is a second dispatcher wiring `findings_path`, reusing the same path template, spawn-time exemption, and inline-fallback unchanged. The on-disk findings this produces are also what let a `plan-architect` `MODE=consult` dispatch cite paths instead of transcribing their contents into the dispatch prompt, per the Model & Effort Routing Opus bullet's pass-by-reference rule. The findings_path-agent contract is enforced only at authoring time, not at dispatch time, and that gap now applies across both dispatchers rather than one.
