...
The same principle was later extended to `agent-review` and `skill-review` — two dispatcher-reached reviewer skills that carry TRIGGER blocks but are always invoked by name from `/code-review` (SKILL.md:241) or the `require-skill-review` hook, with zero observed description-auto-triggers. The additional motivation is stow-consumer budget pressure: because claude-config is stowed to `~/.claude`, every globally-stowed skill description loads in every project's listing; freeing the two descriptions reduces listing pressure in downstream projects. The TRIGGER blocks are kept for graceful degradation on pre-v2.1.129 clients. Because `skill-review` is a plugin-scoped skill (`plugins/skill-management/`), its `skillOverrides` entry uses the qualified `plugin:skill` key `"skill-management:skill-review"`; a bare key silently no-ops for plugin skills (the harness matches on the qualified form it uses in its own listing).
