...
Because "I never use Artifacts or Workflows" is a personal preference, not a claim every stow consumer should accept, the fix belongs in each user's own untracked `~/.claude/settings.local.json`, not this repo's shared `claude/.claude/settings.json` — the same placement `docs/skills.md`'s `skillListingBudgetFraction` guidance already uses.
...
- `docs/skills.md` "Tuning the skill-listing budget for your project" — the `skillListingBudgetFraction`/`settings.local.json` precedent this entry follows.
