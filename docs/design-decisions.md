...
**`require-review-orchestrator-agent-target.sh` closes the one-hop-around gap the Bash restriction leaves open.** `review-orchestrator` is also granted `Agent` (needed to dispatch `code-writer` for fixes and reviewer personas for the skill's own routing), and a Bash restriction alone does nothing to stop it from nest-dispatching an unrestricted `general-purpose` or `claude` to do the mutating and gate-releasing on its behalf — reopening the Bash gate's own gap one hop away, with zero `Bash` call from `review-orchestrator` itself. The allowlist reuses `_LIB_REVIEW_ONLY_AGENTS ∪ {code-writer}` rather than inventing a third roster: every member is already established as mutation-restricted and non-gate-releasing, which is exactly the property a legitimate nested-dispatch target needs.
