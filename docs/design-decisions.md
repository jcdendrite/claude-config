...
Two alternatives were already closed by §28 and are not re-argued here: `permissions.deny: ["Artifact"]`, rejected there because it gates *calling* a tool rather than removing its schema from the system prompt, so it doesn't address the token cost this decision is aimed at; and a settings-file `env` block setting the disable environment variables, which is strictly worse than the settings key directly, since an env-var trigger cannot be cancelled from CLI scope either.
