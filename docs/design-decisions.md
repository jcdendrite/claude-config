...
## 58. `/code-review`'s marker short-circuit left `ready-for-review` untouched (2026-08-24)
...
## 59. Subagent turn-count nudge threshold, and a renewed rejection of a hard per-dispatch cap (2026-08-24)
...
## 60. `marker.sh check code-review` gains an age bound; `write`/commit-gate stay hash-only (2026-08-24)
...
## 54. `guard-settings-session-keys.sh`'s default-branch diff accepts up to 5 extra `_lib_capped` git calls on its staged path (2026-09-04)
...
**Forward warning.** A later entry narrows `skill-fidelity-reviewer`'s SILENT-SKIP ceiling against this widening, once §48's other two grounds are also closed. That narrowing and this widening must be reverted together: reverting this widening alone while the narrowed ceiling stays live would make a fully compliant subagent-dispatched consult produce exactly the false positive the narrowing exists to eliminate.
...
- `docs/design-decisions.md` §25, §48 — the blindness property this entry re-reads, and the out-of-scope declaration this entry reverses. Both read here, neither edited.
...
**§42's "hook-backed in practice" claim narrows to the endorsed branch.** §42 read the plan-file route as converting an advisory rule into a hook-backed one, because the compliance act edits the plan file and re-arms `require-plan-review.sh`. That re-arm now happens only when the consult is endorsed and a plan file exists — a rejected consult produces no plan edit and no re-arm, so the rule stays advisory on that branch, as it always was on the no-plan-file arm. §42 itself is not edited; this entry states the narrower scope its "hook-backed in practice" claim now carries.

**Supersedes §48's SILENT-SKIP verdict-ceiling sentence, on reason 1 only.** §48 held the architect-consult check could never legitimately emit `[SILENT-SKIP]`, for three reasons:

1. `review-trace` never loading subagent records (reason 1).
2. §37's user-ask gate and §42's Fix-route narration both being non-evidence (reason 2).
3. §45's deny-latch correlation being a rejected duplicate detective layer (reason 3).

§58 closed reason 1's scan-visibility half; this entry's unified route closes its plan-file-misclassification half. Together, a `plan-architect` consult prescribed anywhere on the branch's timeline is now observable regardless of route or thread. `skill-fidelity-reviewer.md`'s "The architect-consult check" section is restructured into two cases. The no-deny sub-heading keeps the original zero-rows → *Dismissed as undecidable* rule, reasoned "absence of a row is not evidence of absence," never `[SILENT-SKIP]`. A new gate-deny sub-heading is the one case that now earns `[SILENT-SKIP]`: a `denial` row whose `msg=` text opens `Blocked by architect-consult gate:` with no later `architect-consult` row on the same branch's timeline, decidable because the preventive gate's own deny message reaches the timeline as tool-call metadata, never the deviating session's rationale. **§48 reasons 2 and 3 stand untouched** — neither §37's user ask nor §42's Fix-route narration is observable from the reviewer's evidence surfaces, and this entry narrows the ceiling on reason 1 alone.

**Revert-order coupling with §58, stated canonically here.** §58 and this entry must be reverted together: §58's widened `review-trace` scan and this entry's narrowed `[SILENT-SKIP]` ceiling depend on each other (see the two scenarios below). Reverting this entry alone is safe — the ceiling reverts to its §48 shape and `review-trace`'s widened scan (§58) keeps running unused by this narrower check. Reverting §58 alone while this entry's narrowed ceiling stays live is not: `review-trace` goes blind again to a subagent-dispatched consult, and the narrowed check would then flag a fully §42-compliant subagent-dispatched consult as `[SILENT-SKIP]` — the exact false positive this entry exists to eliminate. The two entries must be reverted together.
...
- `docs/design-decisions.md` §42, §48, §58 — the two-arm routing this entry unifies, the verdict-ceiling sentence this entry supersedes on reason 1 only, and the widening whose forward warning this entry closes. All three read here, none edited.
...
## 60. `attribution.sessionUrl: false` ships in both settings files, reversing §40's `sessionUrl` paragraph (2026-09-05)
...
**No opt-back-in path is confirmed.** The natural candidate — a per-repo `.claude/settings.json` or `.claude/settings.local.json` entry setting `attribution.sessionUrl: true` — is the same open project-scope question above. This repo's CHANGELOG publishes that shape for `disableWorkflows`/`disableArtifact` (§31), which is suggestive but is exactly the cross-key analogy §43 warns against: §43 found `tui`, `theme`, and `agentPushNotifEnabled` *not* honored at a scope engineers assumed worked. The only confirmed recourse today is editing the tracked stow-source file directly, re-clobbered on the next `git pull` unless carried forward — the same disclosure §49's CHANGELOG entry makes for its own reversal path. This entry is the canonical, Revisit-tracked home for this status; the CHANGELOG points here rather than freezing a claim that may go stale.
...
- `docs/design-decisions.md` §40 — the entry this reverses, and the reasoning this entry cites rather than re-derives for `commit`/`pr` staying unset.
- `docs/design-decisions.md` §33 — the precedent for correcting a prior entry's misattribution in a new entry rather than rewriting it in place.
- `docs/design-decisions.md` §41/§49 — the precedent for a reversal entry's structure, including a Revisit list keyed to the superseded entry's own conditions.
- `docs/design-decisions.md` §43 — the cross-key-analogy caution this entry's opt-back-in section applies, and the settings-scope precedent this entry's blast-radius section builds on.
...
## 61. Declined a `SessionStart` + `additionalContext` per-subagent injection mechanism for orchestrator-only CLAUDE.md content (2026-09-03)
