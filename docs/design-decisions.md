...
A prior draft narrowed exactly two Change-type rows (`comment-discipline-reviewer`, `staff-sdet`) to a computed delta-since-last-commit; an engineer objection that treating rows differently was solving the wrong-shaped problem prompted a from-scratch redesign, converged on independently by two model consults. The redesign applies one uniform clause to every Change-type row instead: a spawn's exhaustive-enumeration duty is bounded to the diff already handed to it, but a defect outside that boundary the change causes, activates, or newly reaches stays in scope for the spawn's flagging duty — so no per-row exemption list is needed to protect `ciso-reviewer`, `staff-sdet`, or any other row's cross-change reasoning.

The prior draft also computed a new ref, `git diff HEAD`, to isolate "what changed since the last commit." That ref is wrong — it pulls in unstaged working-tree content, which is neither under review nor covered by the commit-gate marker. This design computes no new ref at all: the boundary is simply the diff a spawn is already being handed (`git diff --cached` for the commit-gate pass, the same basis `require-code-review.sh` hashes), restated as file paths and line ranges for reviewers without `Bash`.
...
The precondition additionally requires `HEAD` not be the repository's default branch. A direct commit to the default branch is followed by no `ready-for-review` cumulative pass at all, so narrowing there would remove the branch's only real review with nothing restoring it — worktree enforcement makes this rare in this repo specifically, but `claude/` installs to every stow consumer and not every consumer opts into worktree enforcement.
