...
**A dispatch, not a hook.** A `PreToolUse` gate on `Write`/`Edit` was rejected on three independent grounds. The tool payload carries a path and content, never a semantic label for "this is a fix I designed mid-review" — there is no predicate at the decision point. A Write-time gate would also fire after the design choice is already made, the wrong moment to catch it. And distinguishing a legitimately-planned new mechanism from an unplanned one would need its own marker to carry that distinction across turns — the identical compounding-layers shape §41 declined for a structurally different case (an unenforceable `ScheduleWakeup` misuse) but for the same reason: a defensive layer whose only job is to close a gap the layer below it created.
...
**The rejected alternative: consult `plan-architect` unconditionally on every ADDRESS finding, no gate.** Weighed directly against this repo's own measured volume: 319 `/code-review` invocations across 120 distinct branches in the 2026-08-01+ window, repo-scoped, against 41 `plan-architect` runs over `--this-repo --since 30d` (both already published in `docs/case-studies/opus-frontload-review-rounds.md`) — an unconditional route puts a ceiling of roughly 9x current `plan-architect` volume, with no corpus instrument today reporting ADDRESS-findings-per-round to narrow that multiplier's floor. It was set aside on three further grounds: it does not remove the contestable judgment, it relocates it to arguing the rule's scope, the same failure this file's existing invalid-rationale lists already document happening elsewhere in this skill; it does not shrink the design, since the consult-vs-revision routing choice and the dispatch's own question both survive, and a third de facto genre inside `MODE=consult` would re-open what §37's explicit mode literal exists to close; and it is inconsistent with *Ripple effect triage* one section earlier in the same file, which resolved the identical dispatch-cost-vs-under-dispatch tension as gate-plus-mandatory-visible-rationale, not spawn-everything.
