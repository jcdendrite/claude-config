...
`plan-review/ROUTING.md` is not a violation of this decision. It is a co-located, single-skill auxiliary file that `plan-review` reads at runtime via the Read tool — it is not shared across skills and creates no cross-skill coupling. Its extraction from `SKILL.md` was driven by `check-skill-length.sh`'s 200-line cap: the routing table grew large enough that keeping it inline would have blocked commits. Co-located runtime auxiliary files of this type are distinct from shared partials because they are scoped to one skill; updating them affects only that skill.
