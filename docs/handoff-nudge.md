...
**Why this block-after count.** HANDOFF_NUDGE_BLOCK_AFTER defaults to 3 (not yet corpus-backtested) — roughly two advisory re-arms of headroom before a hard block; revisit via `rearm-backtest` once escalation-fire data exists. `HANDOFF_NUDGE_BLOCK_AFTER` overrides the count for a consumer whose tolerance for dismissal differs from this default, mirroring `HANDOFF_NUDGE_ABS_CAP`/`HANDOFF_NUDGE_REARM_SPACING` above.
