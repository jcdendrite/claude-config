...
Past a session's first fire, every re-arm the operator lets pass without acting on it increments a per-session ignored-re-arm counter (`<config-dir>/.handoff-nudge-fired.d/<session_id>-ignored`, one byte appended per ignored re-arm). Once that count reaches `HANDOFF_NUDGE_BLOCK_AFTER` (default 4), the *next* re-arm stops being advisory: instead of the `additionalContext` JSON, the hook exits 2 with a reason on stderr — `PostToolBatch`'s own native "stop the agentic loop before the next model call" contract — forcing the session to address the context growth rather than continuing to defer it.

**Why this block-after count.** The block point is `THRESHOLD + HANDOFF_NUDGE_BLOCK_AFTER × HANDOFF_NUDGE_REARM_SPACING`, currently 470000 tokens at the shipped 150000 cap, 80000 spacing, and `BLOCK_AFTER=4`. `HANDOFF_NUDGE_BLOCK_AFTER` counts ignored re-arms, not an absolute token value, so this computed block point needs re-derivation whenever `HANDOFF_NUDGE_ABS_CAP` or `HANDOFF_NUDGE_REARM_SPACING` changes. A single observed session reached 485000 tokens after two dismissed advisory nudges, each dismissal reasonable in isolation, before manual intervention. That session is the sole documented justification for having a hard block at all. `BLOCK_AFTER=4` is the largest ignored-re-arm count whose block point stays under that session's 485000 tokens. [`case-studies/handoff-threshold-impact.md`](case-studies/handoff-threshold-impact.md)'s Tier 0 table recorded 185 `action=block` hard-block firings machine-wide for sessions dated 2026-08-23 or later, against zero for sessions dated 2026-08-22 or earlier. That volume is separate empirical support for `BLOCK_AFTER=4`, alongside the single documented session above.

A corpus scan of 426 fired sessions across 6 accounts corroborates but does not by itself justify any specific value, since it is right-censored — it shows where sessions happened to stop, not where they should have been stopped:

- 60 sessions reach a second fire.
- Only 3 ever reach a fourth crossing.
- None of those 3 show a recorded `/handoff` conversion afterward.

`BLOCK_AFTER=4` trips on a session's fifth crossing — the first fire plus four ignored re-arms. The corpus scan has no figure at that depth. Its deepest figure, 3 sessions reaching a fourth crossing, is already too thin a sample to draw a conclusion from. This corpus scan therefore neither supports nor contradicts `BLOCK_AFTER=4` specifically. Both the grounding session and this corpus scan were measured at a different `HANDOFF_NUDGE_ABS_CAP` than the current 150000 default and have not been re-measured against it: the block points above are recomputed for 150000, but the underlying session and corpus counts are not.

The hard block fires only on `PostToolBatch` — the same hook is also registered on `Stop`, where `exit 2` *forces the conversation to continue* rather than stopping it, so a `Stop`-registered re-arm falls through to the advisory path instead; see "Known limitations" for the resulting coverage gap on `Stop`. `HANDOFF_NUDGE_BLOCK_AFTER` overrides the count for a consumer whose tolerance for dismissal differs from this default, mirroring `HANDOFF_NUDGE_ABS_CAP`/`HANDOFF_NUDGE_REARM_SPACING` above — the stderr message on a hard block names it directly. Revisit this default via an extended `rearm-backtest` once escalation-fire data exists (the `action=block` log field added alongside this default exists for exactly that future analysis) — concretely, once at least two contributors report a hard block on a session they consider legitimate, or after enough elapsed time that fresh corpus data materially changes the counts above.

**Recovering from a hard block.** The existing kill-switch (`touch "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.handoff-nudge-disabled"`) suppresses every future fire, including the hard block, the same as it already suppresses the advisory nudge. Removing `<config-dir>/.handoff-nudge-fired.d/<session-id>-ignored` resets that one session's escalation count (the next re-arm goes back to advisory) without disabling the nudge globally — the `handoff` skill does this automatically as part of recording a successful `/handoff` conversion (see its own SKILL.md), so completing a handoff is the ordinary way out of a block. This is sticky within a session: once the ignored-count has reached `HANDOFF_NUDGE_BLOCK_AFTER`, every later re-arm in that same session hard-blocks too, not just the one that triggered it — the count is monotonic and resets only by one of the two routes above. A third route exists while `/handoff` itself is running: its active-bypass marker (`<config-dir>/.handoff-active.d/<session_id>`) keeps a qualifying re-arm advisory instead of blocking. This is what stops the multi-turn collect-dispatches wait below from being truncated by a re-block. The marker suppresses the block only — it does not reset the escalation count, so a re-arm during this window still counts toward the next block's threshold. `/handoff` collects this session's in-flight background dispatches before writing the file (see the `handoff` skill's own "Before writing: collect in-flight background dispatches" section), so the ordinary exit from a block can take longer than a single turn.
...
- **At the shipped default, `BLOCK_AFTER=4`'s block point exceeds the 200k arm's own window capacity, so the hard block cannot fire there.** The 200k arm's effective threshold is 80000, not the 1M arm's 150000 cap (see the table under "What the hook does"). That puts the block point at 400000 tokens on this arm — above its entire 200000-token window capacity. A consumer who overrides `HANDOFF_NUDGE_BLOCK_AFTER` down (e.g. to 1, giving a block point of 160000) restores reachability on this arm.
