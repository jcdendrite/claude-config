...
`estimate` is the same figure the fire path computes — the sum of the four token fields on the *latest recorded assistant turn*, not a live running count, so it lags by at most one assistant step. `over_threshold` is the field to act on; it uses the same `>=` comparison the fire path does, so the two never disagree about the same session. `already_fired` reports whether the marker file exists — i.e. whether the nudge has fired at least once this session — and `nudge_disabled` reports the kill-switch — **`--check` does not honour the kill-switch**, because that switch suppresses *notifying*, not *measuring*, and a session that explicitly asks for a number should get one. `model_recognized` is `false` only when the model ID matched no arm of the table above and fell to the 1M default, in which case the threshold may not match the running model. `already_fired=true` no longer implies "no further nudge this session": the marker re-arms at escalating bands (see "What the hook does" above), so a session can be both `already_fired=true` and still due for another fire once its estimate advances past the next band.
