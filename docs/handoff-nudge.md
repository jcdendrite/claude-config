...
**Why this block-after count.** The block point is `THRESHOLD + HANDOFF_NUDGE_BLOCK_AFTER × HANDOFF_NUDGE_REARM_SPACING`, currently 470000 tokens at the shipped 150000 cap, 80000 spacing, and `BLOCK_AFTER=4`. `HANDOFF_NUDGE_BLOCK_AFTER` counts ignored re-arms, not an absolute token value, so this computed block point needs re-derivation whenever `HANDOFF_NUDGE_ABS_CAP` or `HANDOFF_NUDGE_REARM_SPACING` changes. A single observed session reached 485000 tokens after two dismissed advisory nudges, each dismissal reasonable in isolation, before manual intervention. That session is the sole documented justification for having a hard block at all. `BLOCK_AFTER=4` is the largest ignored-re-arm count whose block point stays under that session's 485000 tokens. [`case-studies/handoff-threshold-impact.md`](case-studies/handoff-threshold-impact.md)'s Tier 0 table recorded 185 `action=block` hard-block firings machine-wide in its after era, against zero in its before and excluded eras. That volume is separate empirical support for `BLOCK_AFTER=4`, alongside the single documented session above.
...
- **`BLOCK_AFTER=4`'s block point exceeds the 200k arm's own window capacity, so the hard block can never fire there.** The 200k arm's effective threshold is 80000, not the 1M arm's 150000 cap (see the table under "What the hook does"), so `THRESHOLD + HANDOFF_NUDGE_BLOCK_AFTER × HANDOFF_NUDGE_REARM_SPACING` computes to 400000 tokens on that arm — above its entire 200000-token window capacity. No session running a 200k-window model can accumulate enough tokens to reach the hard block, regardless of `HANDOFF_NUDGE_BLOCK_AFTER`.
