...
Revisit this default via an extended `rearm-backtest` once escalation-fire data exists — concretely, once at least two contributors report a hard block on a session they consider legitimate, or after enough elapsed time that fresh corpus data materially changes the picture. The `ignored=`/`skills=` fields recorded on every `nudged` log line (see "Log location" below) exist for exactly that future analysis. PR #769 already raised this default from 1 to 4 (floor 230,000→470,000). [`case-studies/handoff-hard-block-position.md`](case-studies/handoff-hard-block-position.md) measured the block rate falling from 29.8% to 4.2% of nudge fires afterward and cost per shipped PR falling 47.5% alongside it — a clean win, arguing against tightening the floor further. That same study found the share of session dollars spent past the advisory threshold still rising (57.6%→77.3%), so this trigger stays in place rather than relaxing: a future report of a hard block on a session considered legitimate remains the signal to revisit.
...
- **The hard block is unreachable on a 200k-window model.** `HANDOFF_NUDGE_BLOCK_AT`'s default (470000) sits above any 200k context window, so a session on such a model can never reach it — the advisory nudge re-arms indefinitely instead. A future retune of `HANDOFF_NUDGE_BLOCK_AT` toward a 200k-reachable value should be a deliberate choice, not an accident. This unreachability predates PR #782 — PR #769 already raised the floor to 470,000 the day before, for every context-window size, not only 200k (proven arithmetically in [`case-studies/handoff-hard-block-position.md`](case-studies/handoff-hard-block-position.md)). That study also measured the block-fire rate falling from 29.8% to 4.2% of nudge fires once the raised floor took effect, alongside a 47.5% fall in mean cost per shipped PR.
