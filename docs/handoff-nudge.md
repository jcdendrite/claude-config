...
**Why 40%.** Chosen from `transcript-analysis.py context-distribution --since 8d`, run against each of four Claude Code account config directories on 2026-08-07. At the prior 60% threshold, the dollar-share attributable to sessions whose peak context crossed it ranged from 24.6% (lowest account) to 80.8% across the four accounts — most spend happened in sessions that never reached the nudge point at all. At 40% that coverage rose substantially on every account, most dramatically on the lowest-coverage one: 24.6% to 98.0%. 30% was rejected as too aggressive — session-share crossing 30% was already 40–60% of *all* sessions on some accounts, which would make the nudge fire on a majority of sessions regardless of length. 40% is the point where coverage jumps meaningfully without over-triggering. Re-run the same command against current account data before changing this value again — see `## Known limitations` for the dismissal-risk this move trades off.
