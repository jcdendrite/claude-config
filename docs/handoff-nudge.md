...
- **The hard block is unreachable on a 200k-window model.** `HANDOFF_NUDGE_BLOCK_AT`'s default (470000) sits above any 200k context window, so a session on such a model can never reach it — the advisory nudge re-arms indefinitely instead. A future retune of `HANDOFF_NUDGE_BLOCK_AT` toward a 200k-reachable value should be a deliberate choice, not an accident. PR #769, not PR #782, raised the floor to 470,000 one day earlier. [`case-studies/handoff-hard-block-position.md`](case-studies/handoff-hard-block-position.md) proves arithmetically that this unreachability holds for every context-window size, not only 200k — see "Why this block point" above for that study's measured block-rate and cost figures.
