...
Past a session's first fire, every re-arm the operator lets pass without acting on it increments a per-session ignored-re-arm counter (`<config-dir>/.handoff-nudge-fired.d/<session_id>-ignored`, one byte appended per ignored re-arm). Once that count reaches `HANDOFF_NUDGE_BLOCK_AFTER` (default 1), the *next* re-arm stops being advisory: instead of the `additionalContext` JSON, the hook exits 2 with a reason on stderr — `PostToolBatch`'s own native "stop the agentic loop before the next model call" contract — forcing the session to address the context growth rather than continuing to defer it.

**Why this block-after count.** The block point is `THRESHOLD + HANDOFF_NUDGE_BLOCK_AFTER × HANDOFF_NUDGE_REARM_SPACING` (holding the cap and spacing at their own defaults, 150000 and 80000): 230000 tokens at `BLOCK_AFTER=1`, 310000 at 2, 390000 at 3. The primary grounding is a single observed session that reached 485000 tokens after two dismissed advisory nudges, each dismissal reasonable in isolation, before manual intervention: only 1 blocks before that point, since 2 already blocks later than where the session was stopped. A corpus scan of 426 fired sessions across 6 accounts corroborates but does not by itself justify the value, since it is right-censored — it shows where sessions happened to stop, not where they should have been stopped: 60 sessions reach a second fire (the point `BLOCK_AFTER=1` trips) versus only 3 ever reaching a fourth crossing (the BLOCK_AFTER=3 trip point), and none of those 3 shows a recorded `/handoff` conversion afterward. That session and corpus scan were both measured under the prior 360000 cap; the block points above are recomputed for 150000, but the underlying session/corpus counts have not been re-measured against the lower cap's earlier-triggering re-arms. The hard block fires only on `PostToolBatch` — the same hook is also registered on `Stop`, where `exit 2` *forces the conversation to continue* rather than stopping it, so a `Stop`-registered re-arm falls through to the advisory path instead; see "Known limitations" for the resulting coverage gap on `Stop`. `HANDOFF_NUDGE_BLOCK_AFTER` overrides the count for a consumer whose tolerance for dismissal differs from this default, mirroring `HANDOFF_NUDGE_ABS_CAP`/`HANDOFF_NUDGE_REARM_SPACING` above — the stderr message on a hard block names it directly. Revisit this default via an extended `rearm-backtest` once escalation-fire data exists (the `action=block` log field added alongside this default exists for exactly that future analysis) — concretely, once at least two contributors report a hard block on a session they consider legitimate, or after enough elapsed time that fresh corpus data materially changes the counts above.

**Recovering from a hard block.** The existing kill-switch (`touch "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.handoff-nudge-disabled"`) suppresses every future fire, including the hard block, the same as it already suppresses the advisory nudge. Removing `<config-dir>/.handoff-nudge-fired.d/<session-id>-ignored` resets that one session's escalation count (the next re-arm goes back to advisory) without disabling the nudge globally — the `handoff` skill does this automatically as part of recording a successful `/handoff` conversion (see its own SKILL.md), so completing a handoff is the ordinary way out of a block. This is sticky within a session: once the ignored-count has reached `HANDOFF_NUDGE_BLOCK_AFTER`, every later re-arm in that same session hard-blocks too, not just the one that triggered it — the count is monotonic and resets only by one of the two routes above, so there is no in-session escape short of them. `/handoff` collects this session's in-flight background dispatches before writing the file (see the `handoff` skill's own "Before writing: collect in-flight background dispatches" section), so the ordinary exit from a block can take longer than a single turn.
...
| Line prefix | Meaning |
|---|---|
| `nudged session=<id> est=<n> model=<id> window=<n> event=<PostToolBatch\|Stop> [action=block]` | Threshold crossed — first fire or a later re-arm, advisory or hard block alike; nudge emitted. `action=block` is present only on a hard-block fire; an advisory fire carries no `action` field |
...
- **Lowering the threshold increases how often a session sees the nudge at all, not just when.** Session-share (the fraction of *sessions*, not dollars, whose peak crosses the threshold) rose on every measured account when the absolute cap first replaced the flat 40%-of-window threshold: from 1.25x (40.0%→50.0% on one account) to as much as ~4x (7.6%→30.8% on another) — see `absolute-token-handoff-threshold.md` for that measurement. That's a real dismissal-as-noise risk if engineers start tuning the nudge out. Weighed against that: the coverage gain was the entire point of that earlier change (most spend previously happened in sessions that never reached 60% at all), and the re-arm spacing above still bounds repetition within any single session to one nudge per band. A two-tier (informational-then-hard) nudge would reduce this risk; not implemented. The 1.25x–4x figures are from that earlier 360000-introducing transition, not re-measured for the 360000→150000 drop in "Why this cap" above — a further reduction this size plausibly grows session-share and re-arm/hard-block frequency further, but by an unmeasured amount.
...
- **A session whose context growth happens entirely on toolless turns gets no hard-block escalation.** The hard block fires only on `PostToolBatch` (see "Why this block-after count" above for why). `Stop`'s own registration exists to cover exactly the toolless-turn case "What the hook does" describes — a turn with no tool calls for `PostToolBatch` to resolve — so a session that only ever crosses re-arm bands on such turns keeps getting advisory nudges indefinitely, with no hard block regardless of `HANDOFF_NUDGE_BLOCK_AFTER`.
- **Sessions nudged only via `UserPromptSubmit` — an event the current registration no longer includes — go entirely un-nudged, and a regression in that coverage surfaces only via a cross-check, not the log alone.** Group `nudged` lines in `<config-dir>/.handoff-nudge.log` by `session=`, restricted to the slice preceding the log's first `event=PostToolBatch` line (the log carries no timestamp field, so position in the append-only file is the only available boundary), then count sessions whose event set is exactly `{UserPromptSubmit}`. That grouping found 4 of 129 sessions (3.1%), measured 2026-08-25. The denominator counts only sessions with at least one event-tagged `nudged` line in that slice; a majority of the slice's lines predate the `event=` field entirely and can't be classified, so they're excluded rather than assumed. `event=UserPromptSubmit` appears only in that historical slice; it is absent from the log-format table above, since the current registration never emits it. The log records fires and never misses, so a genuine regression in `Stop`+`PostToolBatch` coverage produces no log signal of its own — it is detectable only by cross-referencing `transcript-analysis.py` for sessions that crossed the nudge threshold with no corresponding log line. Treat this as a standing check, not a one-time result: re-run the grouping above against the current log before drawing a conclusion from it.
