...
| Was the study's own first-draft attribution to PR #782 correct? | No — corrected before capture | The floor raise landed with PR #769; #782 the next day is a pure encoding refactor onto the same 470,000 value, proven structurally equivalent at every context-window size |
| Was the study's own second-draft "primary outcome not measurable" verdict correct? | No — a second grounding error, also corrected before publication | The authoring machine's local transcript retention floor had silently advanced past the before-era window; a `gh pr list` cross-check found 19 PRs actually merged there, and a second machine's intact corpus supplied the recovered before-era figures above |
