...
Backgrounding a slow Bash call still does not reach this cause: it converts a blocking wait into an equally long idle wait absent independent work to interleave — the same reason the `background-slow-bash-calls.md` section's second row above already rejected that mechanism. A periodic heartbeat during the wait is not vendor-unavailable the same way, though: `Monitor`'s lack of one here is a consequence of this repo's own `ScheduleWakeup` deny (`docs/design-decisions.md` §49), reversible via the one-line settings edit §49 documents. §49's Accepted residual risk already records an observed instance of the model substituting an unguarded `CronCreate` job as a fallback heartbeat, without asking first. Whether pursuing such a heartbeat is worth its own cost — the reversal's process cost and the unguarded-substitution risk, against whatever this measured sleep-poll share would recover — is unevaluated here; no break-even estimate is offered.
