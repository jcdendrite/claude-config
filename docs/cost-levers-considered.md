...
Backgrounding a slow Bash call still does not reach this cause, per the `background-slow-bash-calls.md` section's second row above. `ScheduleWakeup`'s own documented scope is exclusively `/loop` dynamic-mode iteration pacing ("Reschedules the next iteration of a self-paced `/loop`... you don't call it directly," `code.claude.com/docs/en/tools-reference`), not a generic background-work heartbeat. That conclusion holds independent of this repo's own `permissions.deny` entry (`docs/design-decisions.md` §49): reversing that entry would not make a `Monitor`-wait heartbeat a legitimate call, it would only stop blocking a call still outside the tool's own scope. See §49 for why the heartbeat-free state is treated as the correct outcome, not a gap.
