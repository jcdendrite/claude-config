...
| Lever | Verdict | Measured reason |
|---|---|---|
| Relocate hook-backed and doc-backed rationale out of the two always-loaded `CLAUDE.md` files onto surfaces that load only when the content's own trigger fires (`.claude/rules/`, `pr-description/SKILL.md`) | Adopted; savings land by session shape, and the global file's net landed smaller than estimated | Root `CLAUDE.md`: 182 lines / 13,061 chars → 137 lines / 8,376 chars (-45 lines / -4,685 chars), matching the plan's projection — Phase D's four relocations (skill/agent authoring conventions, settings.json conventions, the PR-merge pointer, the redaction-section trim) all landed as scoped. Global `claude/.claude/CLAUDE.md`: 141 lines / 27,332 chars (this plan's stated baseline) → 145 lines / 27,879 chars (+4 lines / +547 chars net, the opposite of the projected cut) — unrelated intervening commits grew the file by +9 lines / +776 chars in the gap between the plan's baseline measurement and Phase A landing (150 lines / 28,108 chars immediately before Phase A), and Phase A+B's own cuts against that pre-Phase-A state (-5 lines / -229 chars) were smaller than estimated: Phase A dropped its step-4 marker-bullet cut entirely (a pinned test in `claude/.claude/skills/tests/test_skills.py` asserts that sentence survives verbatim), and Phase B was rescoped to keep ticket-prose coverage in the "Ground every choice" category-6 bullet rather than deleting it outright, since no other skill picks that surface up. |

Root cause of the saving being session-shape-split rather than uniform (`docs/cost-levers-considered.md`'s own "Context cost root cause" section above): idle-gap cache rebuilds, this repo's dominant spend driver, scale with rebuild *magnitude* (byte count), not rebuild *frequency*, and rebuild frequency is what byte-count trims do not change. A session that opens no glob the relocated content matches (planning, prose, git-only, analysis) avoids that content's magnitude contribution entirely; a code-editing session in this repo — the typical `claude-config` session shape — pays it back on the first `SKILL.md`/agent/`settings.json` read, so the reliable win for that shape is the unconditional root-file cut, not the relocation.
