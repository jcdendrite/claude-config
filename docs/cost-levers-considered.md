...
- **Re-bucketing `pr-cost`'s ledger past 300,000** finds nothing to bucket: this machine's entire ledger (30 rows, one account) tops out at a mean context of 233,081 tokens per PR — zero rows reach even the existing 250k+ open bucket, let alone a new 300–400k/400–500k/500k+ split. The floor's own censoring effect (`docs/handoff-nudge.md`'s "no data at the depth 470000 implies" line) generalizes exactly as expected: this corpus cannot ground a higher value at all, on this machine's data.
- **Classifying after-era block fires by their `ignored=` count** finds 24 of 25 fires (across the four reachable accounts) carry `ignored=4` — the count a session shows only after receiving and dismissing all four advisory re-arms in sequence (150,000 + 4×80,000 = 470,000 is exact by construction for that path). Zero fires show any lower `ignored` count — the shape that would indicate a session reaching the block zone before the operator had a real chance to act on every prior advisory. (One older-format line predates the `ignored=` field and isn't classifiable. The 25-fire count is higher than the study's own 21, reflecting ordinary corpus growth in the interim, not a contradiction — see the study's own "point-in-time snapshot" caveat.)
