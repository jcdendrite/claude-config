...
| Nudge phrasing (lever 3) | Decline a phrasing change | All three of the phrasing lever's pre-registered thresholds — conversion, block-reach, and re-arms-tolerated at compliance — held on the pooled figures; see the plan for the individual numbers. |
| Nudge→handoff conversion report | Built, not deferred | `_nudge_conversion_from_log` and a new `rearm-backtest` report section compute the adherence rate the phrasing lever's gate needs, as a permanent, rerunnable instrument. |
