...
- **`require-worktree-for-file-writes.sh`** — opt-in per repo (same `.claude/worktree-required` sentinel). When active, denies `Edit`/`Write`/`MultiEdit` on paths in the main working tree; all file edits must land in a linked worktree. Paths under `$HOME/.claude/` are always exempt — the harness and skills write plans, memory, settings, and other infrastructure files there as normal operation, and `~/.claude/` may resolve via stow directory-folding into an opted-in checkout.
