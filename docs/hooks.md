...
- **`require-plan-review.sh`** — blocks `Write`/`Edit`/`MultiEdit`/`ExitPlanMode` when an uncommitted or modified plan file exists in `.claude/plans/` without a current `/plan-review` marker for this session. Committed, unmodified plan files are treated as historical and pass through. Two-marker pattern: an active marker (`~/.claude/.plan-review-active.d/<session_id>`) bypasses the gate for the skill's own Write/Edit calls during review (but not `ExitPlanMode`, which stays blocked until review is complete); a completion marker (`~/.claude/plan-review-markers/<repo-hash>.<session_id>`) allows through once the review is clean. See [Gate deadlock recovery](#gate-deadlock-recovery) for the manual-clear escape hatch.
...
- **`deny-reviewer-tree-mutation.sh`** — always on (no worktree-enforcement opt-in required). Denies `Bash`/`Write`/`Edit`/`MultiEdit` from the closed set of review-only agents (`ciso-reviewer`, the seven other `staff-*` personas, `Explore`, `Plan`) that would mutate the tree under review: git writes, `Write`/`Edit`/`MultiEdit` outside `/tmp` or an `agent-reviews/` findings path, and a closed in-place-edit-tool family (`terraform`/`tofu fmt`, `sed -i`/`perl -i`, `gofmt -w`, `prettier --write`, `eslint --fix`, `ruff format`/`ruff check --fix`, `black`, `isort`, `rustfmt`). Every other agent (main session, `code-writer`, `general-purpose`) passes through unconditionally.
...
- **`enforce-marker-script-shape.sh`** — PreToolUse `Bash` gate that denies any `marker.sh` invocation not exactly matching the allowlisted shape pattern (`~/.claude/scripts/marker.sh <subcommand> <skill>`). Chains of individually-valid marker.sh operations joined by `&&` are permitted (capability-neutral — each op is independently allowlisted or harmless); chains to any other command (except the blessed `git commit` tail), `||`/`;` separators, redirects, and extra arguments remain denied. Defense-in-depth against prompt-injection escalation through the `marker.sh` allow rules.
