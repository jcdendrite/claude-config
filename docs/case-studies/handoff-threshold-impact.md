...
**Short answer.** The nudge's own mechanism-engagement gate (Tier 0) clears cleanly and robustly: the share of session dollars spent after a session has already crossed its fire threshold dropped from 89.5% to 58.3%, and the direction holds at every absolute-cap value tested from 100,000 to 360,000 — no sign flip. The primary cost outcome (Tier 1, cost per shipped PR) does **not** show a clean win: mean cost per claude-config PR rose from $41.89 to $51.87 and the upper quartile widened. The after-era median ($31.63) stays inside the before-era's interquartile range, so this study's own pre-registered "overhead dominance" rule is not triggered. A mediator instrument (Tier 3) explains why: sessions-per-branch and continuation "startup burn" as a share of total dollars both rose monotonically across the transition, consistent with more handoffs happening under the lower cap. Review-quality guardrails (Tier 2) show no decline. Net: the gate got tighter and the overhead grew with it — a real trade, not a free win, and not a regression either.
...
**Findings.** `reviewer-yield --this-repo` has no lower-bound date flag, so its three checkpoints are each cumulative since this repo's transcript history began, not era-isolated: 954 dispatches / 638 findings through 2026-08-16; 1,488 / 1,280 through 2026-08-22; 1,882 / 1,934 unbounded (as of this capture). Differencing adjacent checkpoints recovers each window's own contribution: the excluded window (08-17–08-22) added 534 dispatches and 642 findings; the after window (08-23 onward, as of capture) added 394 dispatches and 654 findings. Findings-per-dispatch, both derived windows, stay above 1 (more than one finding per dispatch on average) — consistent with the before-window's own cumulative rate (638/954 ≈ 0.67, undercounted only because the cumulative denominator includes many older, lower-yield dispatches the differenced windows don't). No quality-loss stop condition is triggered.
...
3. **As-of window, held uniform.** The same 3-day as-of window governs every account and every repo Phase C touches; the after era is not flattered by early capture from whichever account happens to merge fastest.
