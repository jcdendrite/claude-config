...
The full method, including every fix, new attribution surface, and gate below, was pre-registered in `.claude/plans/pr-cost-forensics.md` before any figure in this document was cited. Three sequenced phases: correct one pricing defect and one mislabeled column (Phase 1); add the two attribution surfaces whose absence blocked the dissection — `subagent-mix --per-dispatch` and per-review-round cost on `review-trace` (Phase 2); re-measure against real branch data and author this record (Phase 3). No pipeline-behavior change ships from this study — recommendations are named but deferred to a separate reviewed plan.
...
1. **Review-round count and reviewer fan-out.** Floor: 40.2% — the entire subagent thread, a majority of which is reviewer/writer dispatch work. Very likely the majority of the 59.8% main thread too, since each skill invocation (`/code-review`, `/plan-review`, `/ready-for-review`, `handoff`, and others) loads a large body into a prefix that is then re-read on every subsequent turn. **Confirmed, not merely inferred, this session:** per-review-round cost accounting (the surface Phase 2 built specifically to test this) finds 94.9% of the branch's main-thread dollars fall inside a review-round window, against 5.1% before any skill event fires. Main-thread cost on this branch is predominantly review-loop-driven, not baseline session cost — see "Gate results," row 16.
...
- **`_dispatch_usage_summary` pricing defect (fixed).** The function streamed its JSONL line-by-line and never deduplicated by request ID before pricing, while every other pricing path in the tool does. This double-counted cache-class tokens on any multi-record API response and over-summed `output_tokens` across a run whose values only reach the billed figure on the final record — a roughly 1.8x overstatement on every per-dispatch and per-agent-type dollar figure the function fed (`subagent-mix --per-dispatch`, and — before Phase 2 added its own independent dedup step — the per-round join). Any per-dispatch or per-agent-type dollar total printed by this tool before this fix landed is unreliable and should not be cited forward; see [`design-decisions/plan-architect-consult-mode.md`](../design-decisions/plan-architect-consult-mode.md) for the specific downstream metric this discontinuity affects.
...
4. **Main-thread cost confirmed predominantly review-loop-driven.** Per-review-round accounting (`review-trace --round-cost`) sums to 94.9% of main-thread dollars falling inside some review round's window, against 5.1% in the unattributed pre-loop segment before any skill event fires. Rank #1 in the decomposition above stands; it does not need restating.
5. **Every quantitative claim in this document was re-derived against the command that produces it, named alongside the claim, at the time this document was written** — not carried forward from the plan's own discovery-phase figures, which predate the Phase 1 pricing fix and a week of further corpus growth. Where a figure changed only within measurement noise of the discovery-phase estimate (the token-class and thread splits above moved by roughly 0.2 percentage points), the newer figure is the one cited.
...
**Leg 2 — the account's own branch-activity ranking.** Ranked by active minutes across every branch with local corpus activity on the account under study (19 comparable branches, excluding `HEAD` and the trunk branch as non-comparable), the branch under study ranks **#1 of 19**, roughly **12% above the runner-up**. This branch was not merely expensive in absolute terms — it was the single most active branch on the account by session-active time, by a clear margin.
...
- **Capping reviewer fan-out**, and **narrowing `ready-for-review`'s cumulative pass** to the increment since the last clean review rather than the whole branch diff, are both named candidates a future plan should evaluate against the same review-round cost surface this study's Phase 2 built.
...
- **`.claude/plans/pr-cost-forensics.md`** — this study's own plan, including the full assumption ledger, the pre-registered redaction rule, and the Phase 1/2 instrumentation-fix mechanisms.
- **[`design-decisions/plan-architect-consult-mode.md`](../design-decisions/plan-architect-consult-mode.md)** — the `subagent-mix` `Actual$` discontinuity this study's own Phase 1 fix introduces into that metric's history.
- **[`cost-levers-considered.md`](../cost-levers-considered.md)** — this study's own lever-table entry, including the corpus-wide 92.9%-concurrent idle-gap finding this branch's own 0-of-39 result inverts.
