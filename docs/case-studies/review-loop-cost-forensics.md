...
The full method, including every fix, new attribution surface, and gate below, was pre-registered in `.claude/plans/pr-cost-forensics.md` before any figure in this document was cited. Three sequenced phases: correct one pricing defect and one mislabeled column (Phase 1); add `subagent-mix --per-dispatch` (new) and reuse the already-shipped `review-round-cost` (Phase 2); re-measure against real branch data and author this record (Phase 3). No pipeline-behavior change ships from this study — recommendations are named but deferred to a separate reviewed plan.
...
- **A duplicated instrument, discovered during code review (fixed by removal).** This study built `review-trace --round-cost` in Phase 2, believing no instrument attributed branch dollars to review rounds. That premise was wrong: `review-round-cost` already did this, with a stricter and correct boundary rule. The duplicate was removed and the case study's central claim (the rank #1 confirmation) was re-derived from the pre-existing, correct instrument — see Gate result 4.
