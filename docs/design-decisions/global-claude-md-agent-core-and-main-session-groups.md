...
Known gaps. Gap (d), Bash-mediated writes, is an accepted risk on the basis that the always-loaded stub keeps the prohibition in context. Gaps (e) and (g) are mitigated for Edit and Write in auto mode; MultiEdit and other modes remain residual. The other gaps are open:
...
- (e) A consumer who pulls without re-running `install.sh` after a hook-file addition loses the hook. The shipped `permissions.ask` entry asks in their place for Edit and Write in auto mode; MultiEdit and other modes remain residual.
...
- (g) The hook fails open silently when its own `jq` call fails or is missing: it reads an empty tool name and exits 0 with no stderr. Observed on a copy of the hook. The shipped `permissions.ask` entry asks in their place for Edit and Write in auto mode; MultiEdit and other modes remain residual.
...
The fork and identity-gate residual under Forks is an accepted risk that relies on the post-merge fork spot-check. Gap (d) is an accepted risk. Gaps (a)-(c), (f) and (h) are open. Gaps (e) and (g) are mitigated for Edit and Write in auto mode; MultiEdit and other modes remain residual. All share one ownership record:
...
- Re-review triggers, each with how it is observed:
  ...
  - A settings edit slips through gap (c), (d) or (h), or through gap (e) or (g) by MultiEdit or outside auto mode: not detectable from the hook, which emits no ask and leaves no log. Observed only by transcript review or a report.
