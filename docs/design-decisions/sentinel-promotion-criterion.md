...
1. **State is `type=bool` in the schema, not `enum:`.** A key whose meaning depends on a non-boolean value (e.g. `pr_cost_disclosure`'s `enum:dollars`) has no natural mapping onto a `[y/N]` prompt and stays report-only (account scope) instead. (`type`/`enum:` are `config-keys.psv`'s own column and type-string convention — see [Fifteen sentinel files consolidated into one schema-driven config file](sentinel-config-consolidation.md) and [`docs/config-file.md`](../config-file.md).)
