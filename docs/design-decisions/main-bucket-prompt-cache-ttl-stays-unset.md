...
- A later `--ttl-verdict` run returns `adopt` for either bucket — see `.claude/plans/cache-ttl-tuning-analysis.md` § "Per-root reporting and the ship rule" for what that verdict licenses.
- The vendor changes the main-bucket default.
- The vendor changes the over-limit drop to five minutes.
