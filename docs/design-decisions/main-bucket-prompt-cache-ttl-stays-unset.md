...
**Why not `"1h"`.** The vendor advises API-key and cloud-provider users to set `promptCacheTtl` to `1h`. A value in the stowed file reaches every consumer, not just those. For a subscription consumer it also overrides the vendor's drop to five minutes once usage runs on credits. That keeps the 2x write tier running on metered usage, a state this repo's read cannot isolate. A consumer who wants the vendor's advice, or whose own `--ttl-verdict` run favors a tier their default doesn't give, sets `CLAUDE_CODE_PROMPT_CACHE_TTL` in their own shell environment. Setting it in `~/.claude/settings.json` does not work for a stow consumer, because that file is the stowed file. This is the main-bucket-only override that [main-bucket-prompt-cache-ttl-5m.md](main-bucket-prompt-cache-ttl-5m.md)'s "Consumer recourse, in precedence order" paragraph describes.
