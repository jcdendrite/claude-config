...
The same plan inverted the calculus for `GLOBAL_TRIGGER_PATHS` itself, without splitting anything. Before `select-tests.py` gained a domain rule for `claude/.claude/tests/`, `helpers.py` matched no `DOMAIN_RULES` row and no `CROSS_DOMAIN_EXCEPTIONS` row — `_is_py_source_under_claude_or_plugins` excluded the whole directory through what was then `DELIBERATELY_UNMAPPED_TOP_LEVEL_DIRS` — so any change to `helpers.py` reached the `unmatched-path` branch and `FULL_SUITE_TARGETS` regardless of `GLOBAL_TRIGGER_PATHS` membership. A second module placed beside it would have inherited the identical outcome, which is why the unmatched-path finding alone was never a reason to keep `helpers.py` unsplit — a split would only have changed the reason string printed to stderr. Once `claude/.claude/tests/` became a `select-tests.py` domain, `helpers.py` started matching that row directly. `select_pytest_targets` checks `GLOBAL_TRIGGER_PATHS` before it runs the domain-matching loop, so `helpers.py` still short-circuits to the full suite today. Its `GLOBAL_TRIGGER_PATHS` entry is now load-bearing rather than redundant: removing it would drop `helpers.py`'s selection to `claude/.claude/tests` plus `test_ticket_reference_discipline.py` (the two targets `_is_py_source_under_claude_or_plugins`'s cross-domain exception adds), a severe under-selection for a module 95 files import across all four domains. `select-tests.py`'s own comment on the `GLOBAL_TRIGGER_PATHS` entry and `test_select_tests.py`'s `test_helpers_py_global_trigger_forces_full_suite` record this as a load-bearing regression guard rather than an already-provable fact.
