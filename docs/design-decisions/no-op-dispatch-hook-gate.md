...
- `no (further )?action` (and the pre-existing bare `no action`) has no word boundary, so it also matches inside "actionable" and the plain plural "actions" as an accepted false-positive residual. For example, "There are no further actionable items, but keep monitoring the dashboard." is denied.
- `no work to do` has no trailing word boundary after "do", so it also matches inside ordinary words like "document", "download", and "double-check" as an accepted false-positive residual. For example, "Confirm there is no work to document for this release." is denied.
- `no work to do` has no modifier widening the way `no action` was widened to `no (further )?action`. Phrasings like "no further work to do", "no more work to do", or "no additional work to do" are not caught. This is declined rather than overlooked. No confirmed occurrence uses a modified form, and adding one without a confirmed occurrence would violate the three-source grounding rule above.
