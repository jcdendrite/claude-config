...
- `do(ing|es)? nothing`, `no action`, `no further action`, and `no work to do` can describe another actor's inaction rather than the dispatched agent's own, e.g. "Check whether the retry handler does nothing on the third attempt." These idioms can also appear in a scope-limiting or conditional tail of a prompt that states real work for the dispatched agent, e.g. "Read src/a.py and report findings only. Take no further action." This is an accepted false-positive residual: narrowing any of these idioms further risks losing real no-op coverage.
- No alternative in `NOOP_PHRASE_RE` carries a word boundary on either end, since POSIX ERE has no portable `\b` and `grep -w` is not in POSIX. Every alternative therefore matches inside a longer word: "Review the piano action mechanism before the recital.", "Confirm there is no work to document for this release.", and "The task exists only sometimes." are all denied. A trailing boundary would stop matching forms with real no-op coverage such as "just waiting" and "no actions". A leading boundary is expressible in POSIX ERE, e.g. `(^|[^[:alnum:]])no (further )?action`, but is not applied.
- `no work to do` has no optional modifier, unlike `no (further )?action` (e.g. "no further work to do" is not caught). No confirmed occurrence uses a modified form of "no work to do", and the three-source grounding rule bars adding a modified form without a confirmed occurrence.
...
- GH-1022 — the public issue quoting the prompt that grounds the `no (further )?action` alternative, the third source in the grounding rule above.
