...
- `no work to do` has no widening modifier, unlike `no (further )?action` (e.g. "no further work to do" is not caught). No confirmed occurrence uses a modified form of "no work to do", and the three-source grounding rule bars adding a modified form without a confirmed occurrence.
