...
**The three-source grounding rule for the idiom list.** Every idiom traces to one of exactly three sources, cited in the hook's own header:

1. A phrase appearing verbatim in a confirmed no-op dispatch from the corpus.
2. A phrase `claude/.claude/CLAUDE.md`'s Agent Briefing bullet itself names as a prohibited shape (`occupy the turn`, `hold while other dispatches finish`) — a live source, since the incident session quoted that bullet at itself.
3. A phrase quoted verbatim in a public issue filed against this repository and confirmed by the maintainer as a real occurrence, not a hypothetical the issue author invented.

A closed, sourced list is what keeps this from becoming the open-ended phrase-upkeep problem a prior plan's hook rejection warned against, so a speculative addition grounded in none of the three does not go in the list. A grounded source is necessary but not sufficient. A tool-scoping instruction (e.g. "do not read any files", "no tool calls") restricts tool access rather than stating "do no work" — a materially different shape from every idiom in the list — so it stays excluded regardless of its own corpus provenance.
...
- `do(ing|es)? nothing`, `no action`, `no further action`, and `no work to do` can describe another actor's inaction rather than the dispatched agent's own, e.g. "Check whether the retry handler does nothing on the third attempt." This is an accepted false-positive residual: narrowing any of these idioms further risks losing real no-op coverage.
- `no (further )?action` (and the pre-existing bare `no action`) has no word boundary after "action", so it matches trailing inside "actionable"/"actions", an accepted false-positive residual. For example, "There are no further actionable items, but keep monitoring the dashboard." is denied.
- `no (further )?action` also has no word boundary before "no", so it matches leading inside any word ending in "no" immediately followed by " action", an accepted false-positive residual. For example, "Review the piano action mechanism before the recital." is denied.
- `no work to do` has no word boundary after "do", so it matches trailing inside ordinary words like "document", "download", and "double-check", an accepted false-positive residual. For example, "Confirm there is no work to document for this release." is denied.
- `no work to do` also has no word boundary before "no", so it matches leading inside any word ending in "no" immediately followed by " work to do", an accepted false-positive residual. For example, "The construction crew has considerable casino work to do before opening night." is denied.
- `no work to do` has no widening modifier, unlike `no (further )?action` (e.g. "no further work to do" is not caught). No confirmed occurrence uses a modified form of "no work to do", and the three-source grounding rule bars adding one without one.
...
- GH-1022 — the public issue quoting the prompt that grounds the `no (further )?action` widening, the third source in the grounding rule above.
