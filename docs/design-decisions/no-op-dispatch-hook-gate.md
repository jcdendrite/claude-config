...
- This gate is not adversarial-resistant: a caller deliberately constructing a no-op dispatch that clears the length ceiling or avoids the closed idiom list is not caught, so this gate must not be cited as evidence of an enforced cost or abuse control.
