...
- `no (further )?action` (and the pre-existing bare `no action`) has no word boundary on either side, so it matches trailing inside "actionable"/"actions" and leading inside any word ending in "no" immediately followed by " action", both accepted false-positive residuals. For example, "There are no further actionable items, but keep monitoring the dashboard." and "Review the piano action mechanism before the recital." are both denied.
- `no work to do` has no word boundary at either end of the phrase — missing after "do" and missing before "no" — so it matches trailing inside ordinary words like "document", "download", and "double-check" and leading inside any word ending in "no" immediately followed by " work to do", both accepted false-positive residuals. For example, "Confirm there is no work to document for this release." and "The construction crew has considerable casino work to do before opening night." are both denied.
- `no work to do` has no modifier widening the way `no action` was widened to `no (further )?action` (e.g. "no further work to do" is not caught), because no confirmed occurrence uses a modified form and the three-source grounding rule bars adding one without one.
