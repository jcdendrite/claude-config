...
- No alternative in `NOOP_PHRASE_RE` carries a word boundary on either end, since POSIX ERE has no portable `\b` and `grep -w` is not in POSIX. Every alternative therefore matches inside a longer word: "Review the piano action mechanism before the recital.", "Confirm there is no work to document for this release.", and "The task exists only sometimes." are all denied. Accepted: a portable boundary fix would also stop matching forms with real no-op coverage such as "just waiting" and "no actions".
