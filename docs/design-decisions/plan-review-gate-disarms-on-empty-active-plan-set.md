...
The content this route can surface is bounded to a version of that exact plan path already committed somewhere: a novel plan stays untracked (`git ls-files --others` is base-independent) and stays active, since hiding it from that check needs `git add`, which needs Bash. Severity: Low, but not because every `Write`-holding agent also holds `Bash` — two personas in this repo's roster, `comment-discipline-reviewer` and `skill-fidelity-reviewer`, hold `Write` with no `Bash` (`claude/.claude/agents/*.md` frontmatter). For every other `Write`-holding agent, a Bash file write bypasses this whole gate, so the Write-only route's marginal exposure over that Bash baseline is small. For those two `Bash`-less agents, `deny-reviewer-tree-mutation.sh` closes the route directly: both sit in that hook's `_LIB_REVIEW_ONLY_AGENTS` list (`_lib.sh`). That list confines a review-only agent's `Write` to `/tmp` and `agent-reviews/`, so neither can write to `.claude/plans/`, `MERGE_HEAD`, or `refs/remotes/origin/<default>` in the first place. This rating additionally assumes the actor cannot fabricate a supporting commit without Bash. That is an assumption, not a verified enforcement point — no hook in this repo confirms that an arbitrary agent's `Write` cannot fabricate a commit object the way `deny-reviewer-tree-mutation.sh` confines a review-only agent's `Write` target. `test_hand_forged_anchor_no_real_merge_allows_unrelated_write` pins this route as a tested fact, distinct from the real-`git merge` construction the sibling forged-anchor test drives.
...
Reopen it only when residual 1 (clean-apply zero-gate) or residual 11 (`bash -c`/`eval` wrapping) of `.claude/plans/merge-aware-review-gates.md` § "Out of scope" is closed, per residual 14 there. Accumulated annoyance is not a reopening trigger.
