...
An already-provisioned profile was observed to still have `claude-plugins-official` registered after pulling this commit, confirming the paragraph above. It needs one engineer-run step, once, after pulling: `claude plugin marketplace remove claude-plugins-official`.
