...
Counts and durations are not reproduced here: the corpus mixes this owner's private-project and public-repo transcripts, so publishing a figure from it is barred by `docs/private-project-redaction.md` § "Publishing a tooling measurement". See the upstream reports below for the full evidence.
