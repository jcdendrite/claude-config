...
**Six residuals, named rather than engineered away:**

1. **A narrow false-hit window at write time.** `HEAD^{tree}` changes only via a local commit, rebase, or reset, so the false-hit window is limited to a commit landing between checks passing and the write within one step-2 pass, which the gate's own flow never produces.
