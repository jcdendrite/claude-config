...
`/ready-for-review` (RFR) step 3 runs a mandatory, fully-unnarrowed `/code-review` over the cumulative PR-vs-base diff after every fix, per the Overview's loop rule: a fix produced by step 2, 3, or 4 returns the gate to step 2, and step 3 then re-reviews the fixed cumulative diff in full because its cache marker misses on the changed bytes. That is Reading A — the byte-exact cumulative guarantee stays intact, unlike the discarded Reading B, under which a fix commit's staged-diff review alone would suffice to re-push. Step 4 is the one step that does not re-run on its own output, so a skill-procedural-fidelity finding's own fix does not trigger a second step-4 pass. Nothing in this decision narrows the cumulative pass itself; the four changes below only shorten how many times it has to run before it comes back clean.
...
Each cumulative pass's `/code-review` writes its disposition table, even an empty one, to `agent-reviews/code-review-dispositions-<suffix>.md` at RFR step 3 — the loop's altitude, not `code-review/SKILL.md`'s, since the record is about a branch's history of passes, not about one review. Whether a pass counts as clean or dirty is derived from the rows themselves, with no separate summary line: a status field could drift from the rows it summarizes, and deriving it structurally removes that failure mode entirely rather than adding a check for it.
...
The record is a self-attestation: nothing correlates its write with a review that actually ran, the same residual [§44](ready-for-review-cumulative-diff-cache.md)'s second named residual already accepts for the `cumulative-review` marker itself. The record authorizes nothing on its own — the `cumulative-review` marker stays the sole authorization to skip a re-review — so a forged or stale record can only misinform a later pass's context, never grant it a skip.
...
Dispatching the cap consult (or a contradiction consult) with `MODE=consult` writes the round-3 gate's latch the same way any other `plan-architect` consult does, with no RFR-aware bypass in the recorder. A cap or contradiction consult that fires inside an RFR run therefore permanently spends that branch's one round-3 firing, per the latch's own stated meaning in [§45](round3-plan-architect-consult-gate.md) — "an architect consult ran on this branch recently," not "the round-3 gate's own prescribed consult ran." The two mechanisms are orthogonal by construction rather than by an explicit interlock: the active-marker bypass makes their trigger windows mutually exclusive, since the round-3 gate can only fire outside an RFR run and this cap can only fire inside one.

## How often the new stops interrupt a run

The two new blocking stops — a second consecutive dirty pass, or a finding against an already-settled site — fire only after the loop has already shown it isn't converging, not on ordinary progress. They replace silent thrash: without them, a round-4 finding could revert a round-2 fix with no record either round happened, the shape this decision's own plan names as GH-752's round-4-reverts-round-2 pattern. Autonomous shipping's "without pausing" language covers routine progress through a converging loop, not a loop whose own history shows it is not converging.

## The keep/cap interaction

A *keep* verdict counts as resolved, so a pass whose only open finding resolves *keep* is clean and resets the cap's consecutive-dirty count (ledger row 30). The cap is therefore not expected to fire on the GH-752 revert shape at all — same-site drift is caught first by the contradiction route and the settled-site stop, which bound it independent of the cap's count. The cap's own named residual is new-finding churn across *different* sites: a run alternating dirty passes at distinct sites with clean passes between them never accumulates two *consecutive* dirty passes, so it escapes the cap by the same reset rule that lets *keep* clear it.

## The prose row

`code-writer`'s self-review table gained a row routing comments, docstrings, and durable-doc prose to `comment-discipline-reviewer`'s `## Core review angles` section, and its Step 5 scaling guidance now runs that read on any added or rewritten prose beyond a whitespace or typo fix, even a single line — in a one-paragraph-per-line skill file, one line can be a whole paragraph. Two baseline bullets back it: text a skill or agent loads at runtime never moves into an edit-time-only file such as `REFERENCES.md`, and a change that would push a file past its line cap, or alter a test-pinned clause, stops and reports rather than trimming elsewhere or editing the pin.

## A line-budget assumption went stale mid-implementation

This plan's row 23 measured `code-review/SKILL.md` at 483 lines when the plan was approved, giving items 2 and 4 a 17-line budget against the file's 500-line cap. Before implementation reached that file, an unrelated commit (`8bd5c082`, "Route round-3 architect-consult gate's return by verdict") landed on `origin/main` and added a 12-line "Round-cap architect consult" section to the same file, moving the real baseline to 495. Applied as originally specified, this plan's own +16 lines would have landed at 511 — 11 over the cap.

A `plan-architect` consult, dispatched to resolve the drift, first confirmed the round-cap and cap-consult mechanisms this plan and `8bd5c082` each introduce are orthogonal by construction, not by coincidence: the same active-marker bypass discussed above makes their trigger windows mutually exclusive. The two additions to the same file conflict only in line count, never in behavior. The consult then found a compression that preserves every fact from the plan's original wording at the cost of flattening two bullet lists into prose, landing the file at exactly 500 lines.

That leaves `code-review/SKILL.md` at its 500-line cap with zero headroom — a residual this decision does not close. A future change needing room in that file will have to either raise the cap or extract the Item ownership table into a runtime `ROUTING.md`, following `plan-review`'s existing precedent for that table.

## Sources

- `.claude/plans/rfr-reentry-convergence.md` — full assumption ledger, mechanism list, and out-of-scope residuals.
