...
`code-review/SKILL.md` § "Finding disposition" (the `code-review-contradiction-route` region) now defines "site" and requires `--source "<file:line>"` on a *keep current text* verdict's ledger entry; the field stays optional for every other disposition. The orchestrator matches a finding's location against that definition, and against earlier rounds' fixes and verdicts, by judgment. The requirement is prose-only: `review-ledger.sh` still accepts a *keep current text* entry with `--source` omitted, defaulting the field to `"n/a"` with no rejection path. An omitted `--source` on such an entry leaves no durable anchor for a repeat finding to match against across a session compaction or resume.

These stops replace silent thrash: without them, a round-4 finding could revert a round-2 fix with no record that this happened — the shape GH-752 took. Autonomous shipping's "without pausing" language covers routine progress through passes that hit none of the triggers above, not the stops themselves.
