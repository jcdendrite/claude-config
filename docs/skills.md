...
- **`/sync-pr-description`** — verify and sync an open PR's description against branch state: reader-coherence pass, content-claim verification, coordination-step preservation, backtick hygiene, applied with `gh pr edit`. Dispatched from `/ready-for-review` step 5 and the `/handoff` pre-write checklist; also runs standalone. Model-invocable by exact name; description excluded from the listing budget via `skillOverrides: name-only`.
