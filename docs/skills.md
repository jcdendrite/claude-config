...
- **`/verify-sources`** — when researching or doing web research on a library, API, or architecture decision, or when acting on a claim from a secondary source, confirm it at the official docs or spec directly rather than trusting agent summaries.
