...
- **`/review-loop-cost-audit`** — diagnose whether a branch's review-loop spend is a stuck loop, plan-grinding, or ordinary large-diff work between review gates, by correlating `review-round-cost`'s dated round table against a git-log code-freeze date; a corpus-wide sweep ranks candidates, a single-branch deep audit resolves one to a verdict. Model-invocable by exact name; description excluded from the listing budget via `skillOverrides: name-only`.
...
| `/review-loop-cost-audit` | Diagnose a branch's review-loop spend as a stuck loop, plan-grinding, or ordinary large-diff work, by correlating `review-round-cost`'s dated round table against a git-log code-freeze date: corpus-wide sweep ranks candidates, single-branch deep audit resolves a verdict |
...
- **`pr-description/DEFAULT_TEMPLATE.md` is a runtime auxiliary that needs no read-gate hook.** The skill reads it as one branch of an instruction it must execute to produce any body, not as an elected mid-task read; `docs/design-decisions/no-shared-skill-partials.md` states why.
