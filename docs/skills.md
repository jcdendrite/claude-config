...
- **`/review-loop-cost-audit`** — decompose why a branch's review loop cost what it did and ran as many rounds as it did: where the dollars concentrated (`review-round-cost`'s round table, with round instants from `review-trace`), and what triggered each expensive round (`judgment-pair` plus a bounded narrative read). A code-freeze thrash test against the git log is one flag among several. A corpus-wide sweep ranks candidates; a single-branch deep audit reports shares, causes, and cheaper levers. Model-invocable by exact name; description excluded from the listing budget via `skillOverrides: name-only`.
