...
`<config-dir>/machine-id` shares its trust boundary with the ledger files themselves: anyone with write access to the config directory already has equivalent control over both, so symlink-adoption grants no stronger integrity guarantee than this tool's other local file handling already assumes.
...
**Same per-account opt-in gate as `--all-accounts`.** An account is included only if its own `~/.claude/.pr-cost-enabled` sentinel exists (see "`--all-accounts`" above); an account with no sentinel is skipped, not aborted, with a per-account stderr notice and a count in the provenance line's `skipped_no_sentinel`. This reuses the same existence check, so it inherits the same symlink caveat: symlinking one account's sentinel to another's silently opts both accounts into the export together, defeating the per-account gate. Create each account's sentinel as its own regular file.
...
**Residual correlation and the publication boundary.** `machine` plus the two truncated dates form a light correlation surface across a corpus a third party might independently hold — the same residual `pr-cost`'s own `--all-accounts` read mode already documents above, carried forward here. `machine` can still act as a stable cross-row correlation handle. A row captured before the machine-identity change (see "Machine identity" above) persists in the ledger indefinitely once written, since captures are append-only, and exports in every future run carrying its original operator-chosen value, unredacted. A later recapture of the same PR under a new machine identity (`--record --force --pr N`) does not retire that earlier row: `machine` is part of `_collapse_pr_cost_rows_to_current`'s grouping key, so the recapture survives as its own separate row instead of superseding the legacy one — flagged in turn if its own `machine` value is also non-hex, and not flagged if it's a genuine hex identity. The provenance line's `legacy_machine_value_rows` count flags exactly these rows, so a human reviewing the artifact before distribution has a signal instead of assuming every row's `machine` cell is opaque. There is no principled safe column subset to drop instead: the six-decimal `*_usd` floats and per-class token counts are higher-entropy per-PR fingerprints than the `gh`-sourced integers, so joinability against outside data is the mechanism, not any single column's entropy. **Publishing an export row, or any per-row figure derived from one, to this or any public repository is prohibited** — see `CLAUDE.md`'s "Also redact structural fingerprints and provenance." The one exception: **aggregate PR counts, token counts, cost, and models used, reported in one bucket** may be published with explicit human sign-off; anything beyond that set needs its own sign-off before it ships. Sending the raw row-level file to anyone at all is a decision of the same class as publishing an aggregate: it must be made deliberately, not fall out as a side effect of an analysis session.
