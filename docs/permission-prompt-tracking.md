...
- **"Never leaves the machine" means "no committed-git path," not "immune to local backup/sync tooling."** The log lives inside this git-tracked checkout (the same location every other per-machine hook log uses) and is kept out of `git add -A` only by `.gitignore` — `deny-pii-in-commits.sh`'s always-on credential-value scan is an independent second layer that denies a `git commit` whose staged diff matches a credential shape regardless of file path or a `git add -f` bypass, but neither mechanism defends against a local backup or dotfile-sync tool (Time Machine, iCloud Drive, chezmoi) snapshotting `$HOME` directly off disk.
