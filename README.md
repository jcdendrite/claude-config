...
- **Three-tier redaction** — always-on tracker-ID regex, opt-in user-local blocklist, reviewer discipline for structural and numeric fingerprints. See [Private-project redaction](#private-project-redaction).
...
3. **Reviewer discipline** — structural and numeric fingerprints the hook can't catch (a verbatim RLS policy, a rare column-naming pattern; a precise duration or percentage tied to one identifiable session or account) are a review responsibility, not a mechanical one.
