...
- **Multi-account transcript corpus scope** — `transcript-analysis.py`, `post-crash-sessions`, `analyze-context.py`, and `token-analyzer.py` default to scanning the union of every Claude Code config directory declared in `~/.claude/transcript-config-dirs`, not just the active profile — except `cost --summary`, which is scoped to the active account only: on a machine running several accounts to keep engagements apart, a corpus statistic drawn from just one account's history reads identically to the full history unless the resolved-scope header's root count is checked. Every subcommand states that root count in its resolved-scope header, so a corpus statistic never silently reads as complete when it isn't. See [`docs/transcript-analysis.md`](docs/transcript-analysis.md)'s "Corpus scope: the declared-roots file" section for the per-subcommand mechanics and carve-outs.
- **Leaf-module decomposition over a monolithic CLI script** — `transcript-analysis.py`'s corpus read, scope resolution, redaction, pricing, and rendering logic live in an importable `transcript_analysis/` package; the script itself keeps only `build_parser()`, `main()`, and its subcommand handlers. `token-analyzer.py` and `analyze-context.py` import the package directly instead of `exec`-loading the whole CLI module to reach a handful of private helpers. See [`docs/transcript-analysis-architecture.md`](docs/transcript-analysis-architecture.md).
...
- **`settings.json`** — global settings wiring up the hooks, statusline, and a `permissions.deny` hard floor for `sudo` and secret-file reads (see [Auto mode](#auto-mode)). Configured with **sonnet** as the default model; `/plan-it` Step 5 dispatches its architecture-design synthesis to the Opus-pinned `plan-architect` agent regardless of the session's own model, so a whole-session `--model opus` is reserved for the rarer case where the reads themselves need Opus (Model & Effort Routing section of `CLAUDE.md`). Session-only overrides (model, effortLevel) are intentionally not tracked — use the `ANTHROPIC_MODEL` and `CLAUDE_CODE_EFFORT_LEVEL` env vars, or `/effort max` mid-session.
