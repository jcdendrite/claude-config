...
Moving or renaming this checkout (`mv ~/curent-location/claude-config ~/somewhere-else`) breaks every stow symlink under `~/.claude/` and `~/.local/bin/` at once — the symlinks stow created encode the checkout's location, and since `~/.claude/hooks/`, `~/.claude/skills/`, and the `~/.local/bin/*` wrappers are themselves part of what breaks, nothing is left running inside the checkout that could detect or explain the failure. Two guardrails address this:
