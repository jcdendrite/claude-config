...
Marketplace registration and user-scope `enabledPlugins` install are also available standalone via `~/.claude/scripts/register-marketplace.sh` — `install.sh` calls it once for its own profile, but a machine running multiple Claude Code config profiles (each pointed at its own config directory via `CLAUDE_CONFIG_DIR`) can re-run it per profile: `CLAUDE_CONFIG_DIR=<profile-dir> ~/.claude/scripts/register-marketplace.sh`. See [`docs/scripts.md`](docs/scripts.md) for the full description.
