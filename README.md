...
- **`issue-triage`** — Stateless, report-only triage of a repo's open GitHub issues via `/issue-triage`: batched parallel evidence-gathering (a dedicated `issue-triage-evidence` agent per batch), cross-batch synthesis, and an independent claim-verification pass, delivered as a disposition report written outside the repository. A `PreToolUse` hook denies `gh` write subcommands and off-target `-R`/`--repo` calls while a run's batch-evidence dispatch is in flight. `claude plugin install issue-triage@claude-config --scope project`
