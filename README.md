...
| `deny-network-installs.sh` | `Bash` command that installs a named package, uses `npx`/`bunx`/`uvx`/`pipx run` with an explicit `-y`/`--yes`, or hands fetched content to a shell/interpreter | No clear — no bypass valve; run the specific command outside Claude via the `!` shell escape |
| `deny-unlisted-webfetch-domains.sh` | `WebFetch` to a domain not on `~/.claude/webfetch-allowed-domains.md` | Asks in default/acceptEdits/plan mode; denies outright in auto/bypassPermissions/dontAsk — add the domain to the allowlist, or approve the one-time prompt |
