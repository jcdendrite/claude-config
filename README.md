...
- **Self-consuming continuity files** — `/handoff`/`/brief` write to a durable `~/.claude/` directory, not `/tmp`, so they survive a reboot; a `PostToolUse` `Read` hook mechanically consumes the file the moment it's read directly (the same-session `/clear`-then-read path), while `resume-context.sh` covers the fresh-process path. A separate `PostToolUse` `Edit|Write|MultiEdit` hook chmods the file to `0600` mechanically, rather than depending on the skill's own recipe. See [`docs/hooks.md`](docs/hooks.md) (`consume-durable-continuity-file-on-read.sh`, `harden-durable-continuity-file-mode.sh`) and [Context management](#context-management).
