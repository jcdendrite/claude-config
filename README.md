...
- **`issue-triage`** — Stateless, report-only triage of a repo's open GitHub issues via `/issue-triage`: batched parallel evidence-gathering (a dedicated `issue-triage-evidence` agent per batch), cross-batch synthesis, and an independent claim-verification pass, delivered as a disposition report written outside the repository. The batch-evidence dispatch's standing rules (never invoke a `gh` write subcommand, never target a repo other than the run's resolved target) are the only control. Scoped to public-repo backlogs only — the no-enforcement-hook design relies on this repo's issue backlog already being public. `claude plugin install issue-triage@claude-config --scope project`
