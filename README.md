...
- **Multi-account transcript corpus scope** — `transcript-analysis.py`, `post-crash-sessions`, `analyze-context.py`, and `token-analyzer.py` default to scanning the union of every Claude Code config directory declared in `~/.claude/transcript-config-dirs`, not just the active profile — except `cost --summary`, which is scoped to the active account only: on a machine running several accounts to keep engagements apart, a corpus statistic drawn from just one account's history reads identically to the full history unless the resolved-scope header's root count is checked. Most subcommands state that root count unconditionally; `review-trace` and `skill-invocation` defer the header until a match is found, and `cost --summary` prints no header at all, stating its single-account scope on its own `Scope:` line instead. See [`docs/transcript-analysis.md`](docs/transcript-analysis.md)'s "Corpus scope: the declared-roots file" section.
