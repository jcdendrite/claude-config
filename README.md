...
- **Multi-account transcript corpus scope** — `transcript-analysis.py`, `post-crash-sessions`, `analyze-context.py`, and `token-analyzer.py` all default to scanning the union of every Claude Code config directory declared in `~/.claude/transcript-config-dirs`, not just the active profile: on a machine running several accounts to keep engagements apart, a corpus statistic drawn from just one account's history reads identically to the full history unless the resolved-scope header's root count is checked. Every subcommand states that root count unconditionally. See [`docs/transcript-analysis.md`](docs/transcript-analysis.md)'s "Corpus scope: the declared-roots file" section.
