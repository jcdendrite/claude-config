...
- `pr-link` requires `gh` and network access. `cost-ledger --record` writes to `$CLAUDE_CONFIG_DIR/cost-ledger.md` by default (override with `COST_LEDGER_PATH`) and requires the opt-in sentinel `~/.claude/.cost-ledger-enabled`; every other subcommand, and `cost-ledger`'s own default read mode, is local-only and makes no writes.
