...
| Re-points a read to a different backing table (SQL helper, RLS function, application query, materialized view source, or CDC/change-stream filter switched from table A to table B while both stay populated) | `staff-data-engineer` + `staff-backend-engineer` + `ciso-reviewer` (when the re-pointed read feeds an access-control decision — RLS USING/WITH CHECK, security helper, permission check; skip when feeding only display or analytics output) — audit every flow that INSERTs, UPDATEs, or DELETEs in table A (RPCs, edge functions, triggers, admin tools, background jobs, queue consumers, cron jobs, webhook handlers) to confirm each touches table B symmetrically. DELETE-without-symmetric-DELETE is the highest-severity case: ghost rows appear as live data in the new read source. UPDATE flows that touch only table A leave stale state in table B. INSERT-side dual-write verification alone is insufficient; point-in-time orphan counts confirm past state, not the forward-time invariant |
