...
directory is not guaranteed to exist yet, and `chmod 700` keeps it owner-only
for the file's entire durable "until resumed" lifetime (not just the moment
`resume-context` consumes it). The file's own mode is hardened to `0600` by a
`PostToolUse` hook (`harden-durable-continuity-file-mode.sh`) after the
`Write` call, so it does not depend on this recipe. A fresh session — with no
memory of the originating conversation — must be able to read the file and
execute the work.

<!-- HOOK_TEST_FIXTURE: write-target — the skill test suite executes this exact recipe in an isolated $HOME to verify the directory is created owner-only, not just that the prose says so. Do not duplicate the recipe elsewhere; the test re-reads it from here. -->
```bash
