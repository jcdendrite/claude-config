...
- **Single-row queries need the right tool.** `.single()` (PostgREST)
  *errors* if the result is not exactly one row — use it when the
  ...
  multiple).
- **Stable pagination requires a total-ordered `ORDER BY`** with a
  deterministic tiebreaker (primary key) — without one, `LIMIT`/`OFFSET`
  can shuffle rows between pages.
...
Why: unbounded queries DoS the database and caller memory once data
volume outgrows what dev/test exercises.
...
Why: N+1 loops scale O(N) round-trips — fine at test-data volumes,
ruinous at production volume — and big IN lists add planner cost and
round-trip-size limits on top.
...
Why: `SELECT *` couples read-path performance to schema evolution
instead of localizing that coupling to the callsite that needs the new
column.
...
- **Read-then-write without locking:** in a transaction that reads a
  row, decides based on it, then updates it, use `SELECT ... FOR
  UPDATE` — without the lock, a concurrent writer can change the row
  between your SELECT and UPDATE and your decision runs on stale data.
