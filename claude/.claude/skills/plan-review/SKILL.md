...
3. **Are foundation-correctness tripwires clean?** These fire on observable plan text, not on judgment calls. If any fire, stop — output "Foundation concern: [one sentence]" + "Lighter alternative: [one sentence pointing to source]" as the primary output; do not spawn specialists until the foundation question is resolved.
   ...
   - **Unjustified given.** The ledger's root line names a condition the design treats as fixed that the plan could change, or whose dependence it could remove, by editing any repository's own artifacts. Reach extends past the plan's own Critical-files list and past artifacts a third party merely owns — a peer repository's script counts, even one this plan won't touch. Reach stops at the platform or protocol a mechanism runs on top of (a harness's hook contract, a language runtime, a network protocol): no plan can change what that provides from within it, so a given resting on a platform boundary is genuinely beyond reach. "The engineer already decided it" never qualifies either (`[engineer-verified]` tags that). A condition inside reach the plan deliberately declines to change is not a given — it belongs in **Out of scope** with its reason. Required: name the given, name what puts it inside or outside reach, and state what the design becomes without it. Fires only on plans carrying a ledger; a missing givens line is an author-side gap (B8), not a foundation concern.
...
**An invoked-then-abbreviated skill is a missing evidence base.** When the plan, or the session that produced it, names a skill it invoked, check whether the artifacts that skill's body specifies were produced. Reframing an invoked skill as a "lens," "philosophy," or "principle to keep in mind" rather than a procedure to execute is the pattern to catch. A stated, reasoned abbreviation is not a finding; a silent one is. A rationale the invoked skill's own body already rebuts is not reasoned — re-read that body before accepting one.
