...
| Unexplained import-form deviation in a test (e.g. a namespace import added for spy installation) | When the framework's spy/mock mechanism forces an import form that deviates from house conventions (a mutable binding the spy can replace), keep it and add an inline comment naming the constraint so reviewers read it as required technique, not sloppiness |
