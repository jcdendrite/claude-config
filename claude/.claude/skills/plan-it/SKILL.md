...
**Design doc grep.** After identifying the target files, run `rg -l '<stem>' docs/ 2>/dev/null` where `<stem>` is the basename of each modified file (without extension). If any design docs match, read them before designing. A design doc in-repo for a file being modified is the primary source for its intended behavior — do not rely on reading the implementation alone.

**UI-touch scope check.** If the Step 2 restated goal contains keywords like "fix", "migration", "hygiene", "refactor", "cleanup", or "backfill" AND exploration surfaces modifications to `src/components/`, `src/pages/`, or any file primarily containing user-visible copy (JSX with string literals, CMS content), add this clarifying question in Step 4: "This ticket reads as infrastructure work, but the plan touches UI/copy files (`<list files>`). Is UI/copy change in scope, or should those files be excluded?" Do not proceed to Step 5 until the user answers.
...
**External-pattern grounding.** When the chosen approach invokes a pattern from external documentation (a library, framework, or vendor doc), quote the literal source lines that establish the pattern — not a paraphrase, not a summary, not the section heading. This extends Step 3's grep-the-population rule from in-codebase patterns to external sources. A capitalized pattern name ("the X pattern") lifted from prose without a verbatim source quote is a hazard: names crystallize an interpretation that may not match the source. For billing, payment, and subscription data models — where identifier stability and mapping direction are non-obvious — this grounding is non-optional even for patterns the author considers obvious. A `lookup_key`-based tier resolver, a price-ID→tier mapping, or a subscription-state machine should each carry a comment or inline rationale citing the stability invariant from vendor docs.
...
**Domain-triggered mid-plan-review.** For plans touching billing or payment flows, user-visible UI/copy, or new cron jobs and operational dependencies — invoke `/plan-review` on the *draft* plan before finalizing it (not just at the Step 6 handoff). Pass the in-progress plan text to plan-review; it routes to the correct staff personas via ROUTING.md. This is not a new persona-selection step; plan-review's existing routing is the mechanism. The difference is timing: invoke it at Step 5 (while the design is still malleable) rather than only at Step 6.
