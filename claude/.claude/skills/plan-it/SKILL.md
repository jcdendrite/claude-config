...
The plan file stays live during implementation: the engineer may keep editing their own copy while implementation runs, so the committed copy on this branch can fall behind. Before each phase dispatch and again before `/ready-for-review`, re-read `.claude/plans/<topic-slug>.md` at its authoritative location — the engineer's main checkout when a copy exists there, otherwise this branch's copy — and reconcile any divergence with the engineer before continuing. Deciding what to defer, or drafting the PR body, against a superseded copy ships a half-scoped PR.
