...
below applies to one. Whether the `**/action.yml` / `**/action.yaml`
frontmatter globs above actually match a root-level `action.yml` is
`[unverified]` — see `rule-authoring-conventions.md`'s `**/`
zero-segment-match caveat. Verified against GitHub's Actions
documentation, fetched 2026-09-03 (see `docs/rules-references.md` for
per-claim citations):
