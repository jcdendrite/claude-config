...
- **Detect before creating, in this order:**
  ...
  2. A declared tool is checked ahead of a bare `.venv/` — a repo can carry
     both, and the declared tool still owns that directory.
...
- **An "externally managed environment" error (PEP 668) means the command
  ...
  environment, never reach for `--break-system-packages`.
