...
**The dispatch prompt must include the absolute working directory** (e.g. `Working directory: /absolute/path/to/worktree`). check-runner has no guaranteed cwd of its own; a directory-sensitive command run from the wrong tree produces misleading failures. Do not enumerate environment- or fixture-setup commands in the check-runner command list — `supabase db reset`, migration apply, `docker` start/stop, dependency install, DB seed — check-runner runs checks only; perform setup in the correct directory yourself before dispatching. (check-runner is deliberately a plain `Agent`, not `isolation: worktree` — an isolated worktree is a fresh checkout missing the parent's uncommitted changes and prepared environment, so checks would verify the wrong tree; see `docs/design-decisions.md` §11. Passing an explicit working directory therefore does not conflict with the Agent Briefing rule that forbids a `Working directory:` line for worktree-isolated agents.)
