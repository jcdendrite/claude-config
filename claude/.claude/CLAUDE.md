...
- **Sonnet (default):** all code reading, code writing, and specialist reviewer agents. Requested via `model: sonnet` frontmatter in each agent file — resolution is measured unreliable at the platform level (`staff-*`/`ciso-reviewer` dispatches ran Opus anyway in a nontrivial share of sampled cases), so pass an explicit `model: sonnet` on dispatch too; it costs nothing and helps at any honor rate above zero.
