...
- In a repo with worktree enforcement opt-in (`.claude/worktree-required` committed, or the machine-level `worktree_required` config key resolving true — unioned with the literal `~/.claude` location so a value armed before `CLAUDE_CONFIG_DIR` adoption still activates; see `docs/config-file.md` in the claude-config repo for the file and resolution mechanics), Edit and Write must also target the worktree path — the hook blocks main-tree file writes, but resolving paths to `.claude/worktrees/<branch>/...` up front avoids the round-trip denial.
