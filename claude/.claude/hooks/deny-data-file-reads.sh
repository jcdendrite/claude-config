...
DENY_GATE_LABEL="data-file read"
...
_lib_parse_tool_input_or_deny "could not parse tool-input JSON. Refusing to evaluate the Read under malformed input."
...
case "$FILE_PATH" in
  */Downloads/*)
    emit_deny "Read of '${FILE_PATH}' — the path is under a Downloads/ directory, where data dumps and exports tend to land. Reading it risks pulling PII/PHI into Claude's conversation context. (Armed by ~/.claude/data-file-read-guard.md — see docs/security-hardening.md.)"
    exit 0
    ;;
esac
...
if [ -f "$FILE_PATH" ]; then
  ...
  if [ -n "$FILE_SIZE" ] && [ "$FILE_SIZE" -gt "$_LIB_SIZE_THRESHOLD_BYTES" ] 2>/dev/null; then
    emit_deny "Read of '${FILE_PATH}' — the file is ${FILE_SIZE} bytes, over the 5 MB threshold. A large file of any extension is likely a data dump; Read truncates at 2000 lines but those lines are still PII/PHI. (Armed by ~/.claude/data-file-read-guard.md — see docs/security-hardening.md.)"
    exit 0
  fi
fi
...
while IFS=$'\t' read -r _lineno line; do
  ...
  case "$FILE_PATH" in
    $line)
      emit_deny "Read of '${FILE_PATH}' — the path matches the glob '${line}' in ~/.claude/data-file-read-guard.md, a directory or file shape you flagged as holding PII/PHI. (See docs/security-hardening.md.)"
      exit 0
      ;;
  esac
