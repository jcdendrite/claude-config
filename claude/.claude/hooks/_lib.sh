...
# Boundary classes exclude `.` on both sides so `id_rsa` doesn't match inside `id_rsa.pub`, `.env` doesn't match inside `.env.foo` or `package.env`. `.aws/credentials`, `.docker/config.json`, `.kube/config`, `.config/gh/hosts.yml` are matched directory-qualified, not as bare basenames — `credentials`/`config.json`/`config` alone are too generic to flag.
