...
# This gate's threat model includes a cooperative agent steered by injected content toward an install or curl-pipe-to-shell shape, not only an accidental one; the residual gaps below are limits of text matching against a deliberately obfuscated command, not a claim of safety against one (docs/security-hardening.md).
#
