...
# crosses a measured outlier threshold. See docs/design-decisions.md §61 for
...
# - A mkdir that completes after its own timeout's SIGTERM or the
#   follow-up SIGKILL leaves the cap-kill status (see _lib_capped_for's
#   header in _lib.sh) with the directory still created, orphaning the lock
#   directory.
#   BusyBox returns the mkdir's own status when the mkdir completes and
#   then races the cap. Nothing is orphaned in that case.
...
# - `_scan_turn_count_cached`'s windowed `head -c "$window_offset"` read isn't
...
#   only the sampled one. None carries even `_lib_capped_for`'s SIGTERM/SIGKILL
#   backstop, since that wrapper can only cover a spawned external process, not
#   a bash builtin or redirection. A stalled or unresponsive
...
# Default 340; see docs/design-decisions.md §61 for the corpus basis.
...
# Default 10; see docs/design-decisions.md §61 for the corpus basis.
...
# Default 2000000; see docs/design-decisions.md §61 for the parsing-speed
...
[ -d "$MARKER_DIR" ] || _lib_capped_for 2 mkdir -p "$MARKER_DIR" 2>/dev/null || true
