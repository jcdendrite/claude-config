...
# allow through. Dead or unreadable PID → evict the orphan. A non-empty id
# that is not a safe single path component (e.g. containing "../") skips this
# whole bypass and falls through to the deny below instead of being treated
# as absent: ACTIVE_MARKER concatenates it into a path, so the point of
# validating it is to never build that path, not to grant the allow anyway.
