...
class TestDenyInvisibleCommitContent:
    ...
    def test_grep_absent_from_path_does_not_affect_the_gate(self, tmp_path):
        ...
        (sed/tr and a bash word-walk, no grep); grep is not a dependency of
        this hook at all — a plain, otherwise-allowed `git commit -m x`
        (nothing that trips arm 1 or arm 2) still allows with grep absent
        from PATH. Confirms the gate's fail posture doesn't depend on grep
        being present on PATH; sed/tr below are still a real dependency,
        since the fast-reject's own internal quote-strip needs them."""
