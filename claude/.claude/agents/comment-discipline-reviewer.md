...
1. Write all findings to `<path>` using the **Write tool** — do not use `cat`,
   `echo`, shell heredocs, or Python file writes. Heredocs can overrun the
   shell command-length limit and abort mid-write; Write also creates parent
   directories automatically, and this file write is required regardless of
   the default no-unsolicited-.md-files rule — this instruction IS the
   request.
...
2. Return inline **only** the pointer line:
   ...
   Do not include findings inline when `findings_path` is present — the
   parent reads them from the file, and doing so anyway is a defect.
